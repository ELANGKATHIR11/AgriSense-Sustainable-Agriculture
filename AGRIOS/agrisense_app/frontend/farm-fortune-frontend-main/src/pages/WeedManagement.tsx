...
import { Upload, Camera, Target, AlertCircle, AlertTriangle, Calendar, Brain, BookOpen, Eye, Leaf, Loader2, CheckCircle2, Shield, Volume2 } from "lucide-react";
...
const WeedManagement = () => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        if (!detectionSuccess) {
          ...
          const data = await api.detectWeeds({
  ...
  const handleScoldAnalyze = async () => {
    ...
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/api/weed/analyze', {
        method: 'POST',
        headers: {
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {result.governor_decision && (() => {
                const gd = result.governor_decision;
                ...
                return (
                  <div className="bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-200 p-4 rounded-lg space-y-3">
                    ...
                    {/* Evidence */}
                    {gd.evidence && gd.evidence.length > 0 && (
                      <details className="text-sm">
                        <summary className="cursor-pointer text-indigo-700 font-medium">
                          Evidence ({gd.evidence.length} sources)
                        </summary>

...
import { MessageSquare, Send, RefreshCw, Sparkles, Brain, Shield, Volume2 } from "lucide-react";
...
export default function Chatbot() {
  ...
  useEffect(() => {
    const loadGreeting = async () => {
      try {
        ...
        const data = await api.chatbotGreeting(language);
  ...
  const send = async () => {
    ...
    try {
      ...
      const language = i18n.language || "en";
      const data = await api.chatbotAsk(msg, {
                      ...
                      >
                        <div className="text-xs font-semibold mb-1 opacity-75 flex items-center gap-1 justify-between">
                          <div className="flex items-center gap-1">
                            ...
                            {m.role === "assistant" && m.governor_enhanced && (
                              <span className="flex items-center gap-1 text-indigo-600 dark:text-indigo-400">
                                <Shield className="w-3 h-3" />
                                <span className="text-[10px]">Governor</span>
                        ...
                        {/* Governor evidence accordion */}
                        {m.role === "assistant" && m.evidence && m.evidence.length > 0 && (
                          <details className="mt-2 pt-2 border-t border-indigo-200 dark:border-indigo-600">
                            <summary className="text-xs cursor-pointer text-indigo-600 dark:text-indigo-400 font-medium">
                              📋 Evidence Sources ({m.evidence.length})
                            </summary>
                            <ul className="mt-2 space-y-1">
                              {m.evidence.map((ev, idx) => (
                                <li key={idx} className="text-xs text-gray-700 dark:text-gray-300 flex items-start gap-1">
                                  <span className="text-indigo-500">•</span> {ev}
                                </li>
                              ))}
                            </ul>
                          </details>
                        ...
                        {/* Speak button for illiteracy support */}
                        {m.role === "assistant" && (
                          <button
                            ...
                            onClick={() => {
                              if ('speechSynthesis' in window) {
                                ...
                                const u = new SpeechSynthesisUtterance(m.text.replace(/[🌱💡📋✨⚠️✅]/g, ''));
                                u.rate = 0.85;

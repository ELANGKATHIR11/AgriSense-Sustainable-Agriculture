...
from .crop_database import INDIAN_CROPS_DB, get_crop_info, list_all_crops, search_crops_by_category
from .disease_detector import DiseaseDetectionResult, DiseaseDetector
from .weed_detector import WeedDetectionResult, WeedDetector
...
class VLMEngine:
    ...
    def __init__(
        self, disease_model_path: Optional[str] = None, weed_model_path: Optional[str] = None, use_ml: bool = True
    ):
        """
    ...
    def analyze_disease(
        ...
        try:
            ...
            disease_result = self.disease_detector.detect_disease(image_path, crop_name, expected_diseases)
    ...
    def analyze_weeds(
        ...
        try:
            ...
            weed_result = self.weed_detector.detect_weeds(image_path, crop_name, growth_stage, control_method)
    ...
    def analyze_comprehensive(
        ...
        try:
            # Disease detection
            disease_result = self.disease_detector.detect_disease(plant_image_path, crop_name)

            # Weed detection
            weed_result = self.weed_detector.detect_weeds(field_image_path, crop_name, growth_stage)
    ...
    def _merge_recommendations(
        self, disease_result: DiseaseDetectionResult, weed_result: WeedDetectionResult
    ) -> List[str]:
    ...
    def _estimate_treatment_cost(self, result: DiseaseDetectionResult) -> Dict[str, float]:
        ...
        base_costs = {"fungicide": 800.0, "application": 200.0, "labor": 300.0}

        # Severity multiplier
        multipliers = {"mild": 0.5, "moderate": 1.0, "severe": 1.5, "critical": 2.0}
    ...
    def _estimate_weed_control_cost(self, result: WeedDetectionResult) -> Dict[str, float]:
        ...
        method_costs = {"chemical": 600.0, "mechanical": 800.0, "organic": 400.0, "manual": 1200.0}
    ...
    def _merge_cost_estimates(
        self, disease_cost: Optional[Dict[str, float]], weed_cost: Optional[Dict[str, float]]
    ) -> Dict[str, float]:
    ...
    def _calculate_disease_treatment_success(self, result: DiseaseDetectionResult) -> float:
        ...
        severity_penalty = {"mild": 0.0, "moderate": 0.1, "severe": 0.2, "critical": 0.3}
    ...
    def _calculate_weed_control_success(self, result: WeedDetectionResult) -> float:
        ...
        infestation_penalty = {"none": 0.0, "low": 0.05, "moderate": 0.15, "high": 0.25, "severe": 0.35}

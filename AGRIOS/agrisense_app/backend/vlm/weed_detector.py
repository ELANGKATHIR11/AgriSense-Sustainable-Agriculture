...
from .crop_database import Weed, get_crop_info, get_weeds_for_crop
...
class WeedDetector:
    ...
    def detect_weeds(
        ...
        try:
            ...
            weeds_identified = self._identify_weeds(crop_info, image_analysis, growth_stage)

            # Generate control recommendations
            control_recs = self._generate_recommendations(
                crop_info, weeds_identified, infestation_level, growth_stage, preferred_control
            )
    ...
    def _identify_weeds(self, crop_info, image_analysis: Dict[str, Any], growth_stage: Optional[str]) -> List[Weed]:
    ...
    def _generate_recommendations(
        ...
        recommendations = {"chemical": [], "organic": [], "mechanical": [], "integrated": []}
        ...
        if growth_stage:
            if "early" in growth_stage.lower() or "seedling" in growth_stage.lower():
                recommendations["integrated"].append("Critical stage - weeds most competitive now, prioritize control")
            elif "flowering" in growth_stage.lower():
                recommendations["integrated"].append("Avoid chemicals near flowering to protect pollinators")
    ...
    def _assess_priority(self, infestation_level: WeedInfestationLevel, growth_stage: Optional[str]) -> str:
    ...
    def _estimate_yield_impact(self, infestation_level: WeedInfestationLevel, weeds: List[Weed]) -> str:
    ...
    def _get_control_timing(self, growth_stage: Optional[str], weeds: List[Weed]) -> List[str]:
    ...
    def batch_detect(
        self, image_paths: List[str], crop_name: str, growth_stage: Optional[str] = None
    ...
    def get_field_summary(self, results: List[WeedDetectionResult]) -> Dict[str, Any]:

...
def _is_phi_available() -> bool:
    ...
    try:
        ...
        response = requests.get(f"{endpoint}/api/tags", timeout=timeout / 2)
...
def enrich_chatbot_answer(
    question: str, base_answer: str, crop_type: str = "unknown", language: str = "en", timeout_s: float = None
) -> Optional[str]:
    """
...
def rerank_answers_with_phi(
    question: str, answers: List[Dict[str, Any]], top_k: int = 5, timeout_s: float = None
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        answer_list = "\n".join([f"{i+1}. {a.get('answer', 'N/A')}" for i, a in enumerate(answers[:10])])
...
def generate_contextual_response(
    messages: List[Dict[str, str]], system_prompt: str = None, timeout_s: float = None
) -> Optional[str]:
    """
...
def validate_agricultural_answer(question: str, answer: str, timeout_s: float = None) -> Dict[str, Any]:
    """
...
def generate_greeting(user_name: str = None, time_of_day: str = None, language: str = "en") -> str:
    """
...
def enrich_chatbot_answer(
    question: str, base_answer: str, crop_type: str = "unknown", language: str = "en", timeout_s: Optional[int] = None
) -> str:
    """

...
def load_scold_model():
    ...
    try:
        ...
        model_path = os.getenv("SCOLD_MODEL_PATH", str(Path(__file__).parent.parent.parent / "AI_Models" / "scold"))
        ...
        _scold_model = {"model": model, "processor": processor, "device": device}
        _model_loaded = True
...
def run_inference(image: Image.Image, prompt: str, confidence_threshold: float = 0.5) -> List[Detection]:
    ...
    if model_dict is None:
        ...
        return [Detection(class_name="Model Not Loaded", confidence=0.0, bbox=None, area_percentage=0.0)]
...
async def health_check():
    ...
    return {"status": "healthy", "service": "SCOLD VLM Server", "model_loaded": _model_loaded}
...
async def get_status():
    ...
    return {
        ...
        "available_endpoints": ["/health", "/status", "/api/detect/disease", "/api/detect/weed", "/api/analyze"],
    }
...
async def detect_disease(request: ImageAnalysisRequest):
    ...
    try:
        ...
        prompt = (
            f"Identify any diseases in this {crop_type} image. " f"Provide disease names, severity, and affected areas."
        )

        # Run inference
        detections = run_inference(image=image, prompt=prompt, confidence_threshold=request.confidence_threshold)
        ...
        return AnalysisResponse(
            ...
            metadata={"crop_type": request.crop_type, "analysis_type": "disease_detection"},
        )
...
async def detect_weed(request: ImageAnalysisRequest):
    ...
    try:
        ...
        return AnalysisResponse(
            ...
            detection_count=len(detections),
            processing_time_ms=processing_time,
            metadata={"crop_type": request.crop_type, "analysis_type": "weed_detection"},
        )
...
async def analyze_image(request: ImageAnalysisRequest):
    ...
    try:
        ...
        return AnalysisResponse(
            ...
            detection_count=len(detections),
            processing_time_ms=processing_time,
            metadata={"crop_type": request.crop_type, "analysis_type": "general"},
        )

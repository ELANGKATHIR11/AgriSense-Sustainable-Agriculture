...
class HybridAgriAI:
    ...
    def _check_availability(self):
        ...
        try:
            ...
            resp = requests.get(f"{self.config.phi_endpoint}/api/tags", timeout=5)
        ...
        try:
            ...
            resp = requests.get(f"{self.config.scold_endpoint}/health", timeout=5)
    ...
    def analyze_image(
        ...
        try:
            ...
            # Call SCOLD VLM
            payload = {"image": image_b64, "prompt": prompt, "confidence_threshold": self.config.confidence_threshold}

            response = requests.post(
                f"{self.config.scold_endpoint}/api/analyze", json=payload, timeout=self.config.timeout
            )
    ...
    def _build_visual_prompt(self, analysis_type: AnalysisType) -> str:
        ...
        return prompts.get(analysis_type, "Analyze this agricultural image and provide detailed observations.")

    def _fallback_visual_analysis(self, image_data: Any, analysis_type: AnalysisType) -> VisualAnalysis:
    ...
    def analyze_text(
        self, query: str, context: Optional[Dict[str, Any]] = None, use_history: bool = True
    ) -> TextualAnalysis:
        """
        ...
        try:
            ...
            payload = {
                ...
                "stream": False,
                "options": {"temperature": self.config.temperature, "top_p": 0.95, "top_k": 50, "repeat_penalty": 1.1},
            }

            response = requests.post(f"{self.config.phi_endpoint}/api/chat", json=payload, timeout=self.config.timeout)
            response.raise_for_status()
            result = response.json()
    ...
    def _extract_recommendations(self, response: str) -> List[str]:
        ...
        for line in lines:
            ...
            if line and (
                line[0].isdigit()
                or line.startswith(("- ", "• ", "* "))
                or any(word in line.lower() for word in ["should", "must", "need to", "recommend"])
            ):
                # Clean up formatting
    ...
    def analyze_multimodal(
        self, image_data: Union[str, bytes, Image.Image], text_query: str, context: Optional[Dict[str, Any]] = None
    ) -> HybridAnalysis:
        """
        ...
        synthesis = self._synthesize_results(visual_result, textual_result, text_query)

        # Step 5: Extract actionable steps
        actionable_steps = self._extract_actionable_steps(visual_result, textual_result)
        ...
        logger.info(f"✅ Hybrid analysis complete: " f"{processing_time:.0f}ms, confidence={confidence:.2f}")
    ...
    def _build_enhanced_query(self, original_query: str, visual_result: VisualAnalysis) -> str:
    ...
    def _synthesize_results(self, visual: VisualAnalysis, textual: TextualAnalysis, original_query: str) -> str:
    ...
    def _extract_actionable_steps(self, visual: VisualAnalysis, textual: TextualAnalysis) -> List[str]:
    ...
    def _calculate_confidence(self, visual: VisualAnalysis, textual: TextualAnalysis) -> float:
...
def analyze_farm_image(image_path: str, question: str, context: Optional[Dict] = None) -> HybridAnalysis:
    """
...
def ask_agricultural_question(question: str, context: Optional[Dict] = None) -> str:
    """

...
class SCOLDVLMService:
    ...
    def __init__(
        ...
        """
        self.base_url = base_url or os.getenv("SCOLD_BASE_URL", "http://localhost:8001")
        self.model_path = model_path or os.getenv(
            "SCOLD_MODEL_PATH", str(Path(__file__).parent.parent.parent / "AI_Models" / "scold")
        )
        self.confidence_threshold = float(os.getenv("SCOLD_CONFIDENCE_THRESHOLD", str(confidence_threshold)))
    ...
    def assess_crop_health(
        self, image_data: Union[str, bytes, Image.Image], crop_type: Optional[str] = None
    ) -> SCOLDAnalysisResult:
        """
    ...
    def _analyze_image(
        ...
        try:
            ...
            # Call SCOLD VLM API
            response = requests.post(f"{self.base_url}/api/analyze", json=payload, timeout=self.timeout)
    ...
    def _fallback_analysis(
        ...
        return SCOLDAnalysisResult(
            ...
            metadata={"error": "SCOLD VLM unavailable", "context": context or {}, "fallback": True},
...
def analyze_disease_with_scold(
    image_data: Union[str, bytes, Image.Image], crop_type: Optional[str] = None
) -> Dict[str, Any]:

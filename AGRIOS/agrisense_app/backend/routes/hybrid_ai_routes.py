    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "context": {"crop_type": "tomato", "location": "greenhouse", "weather": "humid"},
            }
        }
    ...
    **Returns:**
    ...
    try:
        ...
        result: HybridAnalysis = ai.analyze_multimodal(
            image_data=image_bytes, text_query=request.query, context=request.context
        )
    ...
    try:
        ...
        if context_json:
            ...
            try:
                ...
                raise HTTPException(status_code=400, detail="Invalid JSON in context_json")

        # Convert to base64 for consistency
        image_b64 = base64.b64encode(image_bytes).decode("utf-8")

        # Use main analysis endpoint
        request = MultimodalRequest(image_base64=image_b64, query=query, context=context)
...
async def analyze_text(request: TextQueryRequest):
    ...
    **Examples:**
    ...
    try:
        ...
        result = ai.analyze_text(query=request.query, context=request.context, use_history=request.use_history)
    ...
    **Analysis Types:**
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Invalid analysis_type. Must be one of: {[t.value for t in AnalysisType]}"
            )

        result = ai.analyze_image(image_data=image_bytes, analysis_type=analysis_type, prompt=request.custom_prompt)

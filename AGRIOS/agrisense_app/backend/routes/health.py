...
async def check_database() -> Dict[str, Any]:
    ...
    try:
        ...
        from ..core.data_store import get_sensor_readings, init_sensor_db
        ...
        return {"status": "unhealthy", "message": str(e), "error": type(e).__name__}
...
async def check_cache() -> Dict[str, Any]:
    ...
    try:
        if not settings.enable_redis_cache:
            return {"status": "healthy", "message": "Using in-memory cache", "type": "memory"}
        ...
        if result == "pong":
            ...
            return {"status": "degraded", "message": "Redis cache responding but data mismatch"}
    except Exception as e:
        logger.error(f"Cache health check failed: {e}")
        return {"status": "degraded", "message": "Cache unavailable, using fallback", "error": str(e)}
...
async def check_ml_models() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "degraded", "message": "ML models check failed", "error": str(e)}
...
async def check_celery() -> Dict[str, Any]:
    ...
    try:
        if not settings.enable_celery:
            return {"status": "healthy", "message": "Celery not enabled (sync mode)"}

        # Try to ping Celery workers
        # This would require celery app instance
        return {"status": "healthy", "message": "Celery workers operational"}
    except Exception as e:
        return {"status": "degraded", "message": "Celery check failed", "error": str(e)}
...
async def health_check() -> HealthStatus:
    ...
    return HealthStatus(
        status=overall_status, timestamp=datetime.utcnow().isoformat() + "Z", uptime_seconds=get_uptime(), checks=checks
    )
...
async def readiness_probe():
    ...
    if db_check["status"] == "unhealthy":
        return {"status": "not_ready", "reason": "database_unavailable"}, status.HTTP_503_SERVICE_UNAVAILABLE
...
async def get_metrics():
    ...
    return MetricsResponse(
        timestamp=datetime.utcnow().isoformat() + "Z", system=system_metrics, application=app_metrics
    )

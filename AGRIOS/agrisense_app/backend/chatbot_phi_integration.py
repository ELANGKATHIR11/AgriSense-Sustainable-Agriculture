...
Tone: Friendly, professional, and empathetic to farmers' challenges."""
    ...
    def _call_phi_llm(self, prompt: str) -> Optional[str]:
        ...
        try:
            ...
            response = requests.post(f"{self.config.endpoint}/api/generate", json=payload, timeout=self.config.timeout)
    ...
    def ask(
        self, question: str, context: Optional[Dict] = None, session_id: Optional[str] = None, language: str = "en"
    ) -> Dict[str, Any]:
        """
        ...
        if phi_response:
            ...
            self.conversation_history.append(
                {"question": question, "answer": answer, "context": context, "timestamp": time.time()}
            )
...
def enhance_with_phi(question: str, base_answer: str, context: Optional[Dict] = None) -> Dict[str, Any]:
    """
...
Enhanced answer:"""
        ...
        if enhanced:
            return {"answer": enhanced, "original_answer": base_answer, "enhanced": True, "source": "phi_enhanced"}

    # Fallback: return original
    return {"answer": base_answer, "original_answer": None, "enhanced": False, "source": "knowledge_base"}

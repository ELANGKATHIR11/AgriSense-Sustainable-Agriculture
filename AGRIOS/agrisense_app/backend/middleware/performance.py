...
class PerformanceMonitoringMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            logger.error(
                f"Request failed: {request.method} {request.url.path} " f"after {process_time:.3f}s - {str(e)}"
            )
            raise
...
class IntelligentCachingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        self.cache_paths = cache_paths or ["/api/sensors", "/api/analytics", "/api/recommendations", "/api/weather"]
        ...
        if cache_key in self._cache:
            ...
            if self._is_cache_valid(cached_time):
                ...
                response = JSONResponse(
                    ...
                    headers={"X-Cache": "HIT", "X-Cache-Age": f"{(datetime.now() - cached_time).total_seconds():.1f}"},
                )
                return response
            else:
                # Cache expired

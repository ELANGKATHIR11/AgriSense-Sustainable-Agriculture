...
from .celery_config import TaskResult, celery_app, get_active_tasks, get_worker_stats, validate_celery_config
from .tasks.data_processing import aggregate_sensor_data, process_individual_reading, process_sensor_data_batch
from .tasks.ml_tasks import batch_model_inference, model_performance_evaluation, retrain_models
from .tasks.notification_tasks import send_alert_notification, send_email_notification
from .tasks.report_generation import generate_custom_report, generate_daily_report, generate_weekly_report
from .tasks.scheduled_tasks import backup_database, system_health_check, update_weather_cache
...
async def get_scheduled_tasks():
    ...
    try:
        if not celery_app:
            return {"scheduled_tasks": [], "message": "Celery not available"}
        ...
        if not beat_schedule or not hasattr(beat_schedule, "items"):
            return {"scheduled_tasks": [], "message": "No scheduled tasks configured"}

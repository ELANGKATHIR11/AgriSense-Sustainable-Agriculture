...
class ExplainableAI:
            ...
            if hasattr(self.model, "predict_proba"):
                ...
                if background_data is not None:
                    ...
                    try:
                        ...
                        self.explainer = shap.KernelExplainer(self.model.predict_proba, mock_data)
            else:
                # Regressor
                if background_data is not None:
                    ...
                    try:
                        ...
                        mock_data = np.zeros((10, len(self.feature_names)))
                        self.explainer = shap.KernelExplainer(self.model.predict, mock_data)
    ...
    def setup_lime_explainer(self, training_data: np.ndarray, class_names: Optional[List[str]] = None):
        """
    ...
    def explain_prediction(
        ...
        if isinstance(input_features, dict):
            features_array = np.array([input_features.get(name, 0.0) for name in self.feature_names]).reshape(1, -1)
        ...
        nl_explanation = self._generate_natural_language_explanation(
            prediction, confidence, feature_contributions[:top_k], feature_dict
        )

        # Generate actionable insights
        actionable_insights = self._generate_actionable_insights(
            prediction, feature_contributions[:top_k], feature_dict
        )

        # Generate alternative scenarios
        alternative_scenarios = self._generate_alternative_scenarios(feature_dict, feature_contributions[:top_k])
    ...
    def _explain_with_shap(
        self, features_array: np.ndarray, feature_dict: Dict[str, float]
    ...
    def _explain_with_lime(
        ...
        try:
            ...
            explanation = self.lime_explainer.explain_instance(
                features_array[0], predict_fn, num_features=len(self.feature_names)
            )
            ...
            for feature_desc, contribution in lime_contributions:
                ...
                if feature_name in self.feature_names:
                    contribution_percent = (
                        (abs(contribution) / total_contribution * 100) if total_contribution > 0 else 0
                    )
    ...
    def _explain_rule_based(
        self, feature_dict: Dict[str, float], prediction: Optional[Any] = None
    ) -> List[FeatureContribution]:
        """
    ...
    def _generate_natural_language_explanation(
        ...
        if self.model_type == "classifier":
            explanation_parts.append(f"The model predicts class {prediction} with {confidence*100:.1f}% confidence.")
        else:
    ...
    def _generate_actionable_insights(
        self, prediction: Any, top_features: List[FeatureContribution], feature_dict: Dict[str, float]
    ) -> List[str]:
        """
        ...
        for fc in top_features:
            if fc.feature_name in self.OPTIMAL_RANGES:
                ...
                if value < optimal_min:
                    ...
                    insights.append(f"✅ {feature_desc.capitalize()} is within optimal range - maintain current levels")

        return insights[:5]  # Limit to top 5 insights

    def _generate_alternative_scenarios(
        self, feature_dict: Dict[str, float], top_features: List[FeatureContribution]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for fc in top_features[:3]:  # Top 3 features
            ...
            alt_array = np.array([alt_features.get(name, 0.0) for name in self.feature_names]).reshape(1, -1)
...
def explain_model_prediction(
    ...
    explanation = explainer.explain_prediction(input_features, method=method_enum, top_k=5)

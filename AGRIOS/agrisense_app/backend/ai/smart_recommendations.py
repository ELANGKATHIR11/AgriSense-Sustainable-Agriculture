...
class YieldPredictor:
    ...
    def predict_yield(
        ...
        # Base yield (kg/ha)
        base_yields = {"tomato": 60000, "wheat": 6000, "rice": 8000}
        base_yield = base_yields.get(crop_type, 10000)

        # Calculate efficiency factors (0-1) for each parameter
        moisture_efficiency = cls._efficiency_factor(soil_moisture, optima["soil_moisture"])
        temp_efficiency = cls._efficiency_factor(temperature, optima["temperature"])
        n_efficiency = cls._efficiency_factor(nitrogen, optima["nitrogen"])
        p_efficiency = cls._efficiency_factor(phosphorus, optima["phosphorus"])
        k_efficiency = cls._efficiency_factor(potassium, optima["potassium"])
...
class WaterUsageEstimator:
    ...
    BASE_WATER_REQUIREMENTS = {"tomato": 4000, "wheat": 2000, "rice": 8000, "corn": 3000}
    ...
    def estimate_water_usage(
        ...
        total_requirement = base_requirement * temp_factor * humidity_factor * moisture_factor * field_size
...
class CostEstimator:
    ...
    @staticmethod
    def calculate_irrigation_cost(water_volume: float, water_cost_per_liter: float, energy_cost: float = 0.1) -> float:
    ...
    def calculate_fertilizer_cost(
        ...
        return nitrogen_kg * n_cost + phosphorus_kg * p_cost + potassium_kg * k_cost

    @staticmethod
    def calculate_labor_cost(hours: float, hourly_rate: float) -> float:
...
class SmartRecommendationEngine:
    ...
    def optimize_irrigation_schedule(self, farm_params: FarmParameters, days_ahead: int = 7) -> OptimizationResult:
        """
        ...
        def objective(irrigation_schedule: np.ndarray) -> float:
            ...
            irrigation_cost = self.cost_estimator.calculate_irrigation_cost(
                total_water, farm_params.water_cost_per_liter
            )
        ...
        # Constraints
        constraints = [
            {"type": "ineq", "fun": lambda x: farm_params.max_water_daily_liters * days_ahead - np.sum(x)},
            {
                "type": "ineq",
                "fun": lambda x: farm_params.available_budget
                - self.cost_estimator.calculate_irrigation_cost(np.sum(x), farm_params.water_cost_per_liter),
            },
        ]
        ...
        if result.success:
            ...
            total_cost = self.cost_estimator.calculate_irrigation_cost(total_water, farm_params.water_cost_per_liter)
            ...
            for day, water in enumerate(optimal_schedule, 1):
                if water > 100:  # Only recommend if significant
                    recommendations.append(
                        f"Day {day}: Irrigate {water:.0f} liters " f"({water/farm_params.field_size_hectares:.0f} L/ha)"
                    )

            # Trade-offs analysis
            trade_offs = {
                "yield_vs_water": (
                    f"Using {total_water/1000:.1f}m³ water to achieve " f"{expected_yield:.0f} kg/ha yield"
                ),
                "yield_vs_cost": (
                    f"Spending ${total_cost:.2f} to achieve "
                    f"{expected_yield:.0f} kg/ha yield (${total_cost/expected_yield:.4f}/kg)"
                ),
                "water_vs_cost": (f"Water efficiency: ${total_cost/(total_water/1000):.2f}/m³"),
            }
            ...
            return OptimizationResult(
                ...
                parameters={f"day_{i+1}_irrigation_liters": round(val, 2) for i, val in enumerate(optimal_schedule)},
    ...
    def optimize_fertilizer_plan(
        self, farm_params: FarmParameters, current_nutrient_levels: Dict[str, float]
    ) -> OptimizationResult:
        """
        ...
        bounds = [(0, 200), (0, 150), (0, 200)]  # Nitrogen  # Phosphorus  # Potassium
        ...
        if result.success:
            ...
            if n_add > 10:
                recommendations.append(
                    f"Apply {n_add:.1f} kg/ha Nitrogen " f"({n_add * farm_params.field_size_hectares:.1f} kg total)"
                )
            if p_add > 10:
                recommendations.append(
                    f"Apply {p_add:.1f} kg/ha Phosphorus " f"({p_add * farm_params.field_size_hectares:.1f} kg total)"
                )
            if k_add > 10:
                recommendations.append(
                    f"Apply {k_add:.1f} kg/ha Potassium " f"({k_add * farm_params.field_size_hectares:.1f} kg total)"
                )

            if not recommendations:
                recommendations.append("Current nutrient levels are optimal. No fertilizer application needed.")

            # Trade-offs
            trade_offs = {
                "yield_vs_cost": (f"Spending ${total_cost:.2f} to achieve " f"{expected_yield:.0f} kg/ha yield"),
                "cost_per_kg_yield": f"${total_cost/expected_yield:.4f}/kg",
                "environmental_impact": (
                    "Low" if total_n < 180 else "Moderate" if total_n < 220 else "High (nitrate leaching risk)"
                ),
            }
...
def optimize_fertilization(
    ...
    current_nutrients = {"nitrogen": current_nitrogen, "phosphorus": current_phosphorus, "potassium": current_potassium}

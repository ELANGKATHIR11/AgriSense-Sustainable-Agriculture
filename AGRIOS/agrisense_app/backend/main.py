...
try:
    ...
    VLM_AVAILABLE = True
except Exception as e:  # catch broader errors (tensorflow/transformers may raise non-ImportError)
...
try:
    ...
    from .models import Recommendation, SensorReading
    ...
    try:
        from models import Recommendation, SensorReading  # type: ignore
    except Exception:
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Could not import engine module: {e}")
            RecoEngine = None

    if SensorReading is None:
        # Try one last time via direct import if above failed
        try:
            import models
...
try:
    if __package__:
        ...
        from .database_enhanced import cleanup_database, initialize_database
        from .metrics import MetricsMiddleware, metrics, record_health_check
        from .rate_limiter import (
            RateLimitMiddleware,
            cleanup_rate_limiting,
            initialize_rate_limiting,
        )
        from .tensorflow_serving import (
            cleanup_tf_serving,
            get_tf_serving_status,
            initialize_tf_serving,
        )
        from .websocket_manager import manager as websocket_manager
        from .websocket_manager import (
            periodic_status_broadcast,
            websocket_router,
        )
    else:
        from auth_enhanced import (  # type: ignore
            UserCreate,
            UserRead,
            UserUpdate,
            auth_backend_cookie,
        ...
        from database_enhanced import cleanup_database, initialize_database  # type: ignore
        from metrics import MetricsMiddleware, metrics, record_health_check  # type: ignore
        from rate_limiter import RateLimitMiddleware, cleanup_rate_limiting, initialize_rate_limiting  # type: ignore
        from tensorflow_serving import cleanup_tf_serving, get_tf_serving_status, initialize_tf_serving  # type: ignore
        from websocket_manager import manager as websocket_manager  # type: ignore
        from websocket_manager import periodic_status_broadcast, websocket_router
...
try:
    ...
    from agrisense_app.backend.core.cache_manager import get_cache, initialize_cache
...
# Skip CORS here - configured later with proper origins
try:
...
# If optional optimization bundle isn't available, still try to include
# lightweight AI routes (rule-based fallbacks) so core API remains useful
try:
    if not OPTIONAL_OPTIMIZATIONS_AVAILABLE:
        try:
            # Import the lightweight ai routes we've added for fallbacks
            if __package__:
                from .routes.ai_routes import router as ai_router
            else:
                from routes.ai_routes import router as ai_router  # type: ignore

            # Mount without prefix since middleware strips /api/
            # Incoming /api/v1/... → middleware strips to /v1/... → router matches
            app.include_router(ai_router, prefix="")
            logger.info("✅ Lightweight AI fallback routes included (middleware handles /api/ stripping)")
        except Exception as _ai_exc:
            logger.warning("Could not include fallback AI routes: %s", _ai_exc)
except Exception:
    pass
...
try:
    if __package__:
        ...
        from routes import crop_routes  # type: ignore

    app.include_router(crop_routes.router, prefix="/api/crops", tags=["Crop Recommendation"])
    logger.info("✅ Crop Recommendation router included")
except Exception as e:
    logger.warning(f"Could not include Crop Recommendation router: {e}")
...
# CORS: Configure to allow both localhost and 127.0.0.1 from frontend
_origins_env = os.getenv("ALLOWED_ORIGINS", "")
if _origins_env:
    _allow_origins = [o.strip() for o in _origins_env.split(",") if o.strip()]
else:
    # Default: Allow frontend on both localhost and 127.0.0.1
    _allow_origins = [
        "http://localhost:3000",
        "http://127.0.0.1:3000",
        "http://localhost:8080",
...
# Include AGRI-OS Edge-Autonomous System router
try:
    if __package__:
        from .agrios.router import router as agrios_router
    else:
        from agrios.router import router as agrios_router  # type: ignore

    app.include_router(agrios_router, prefix="/agrios", tags=["AGRI-OS"])
    logger.info("✅ AGRI-OS router included successfully - Edge-Autonomous System available at /agrios")
except Exception as e:
    logger.warning("⚠️ AGRI-OS system not available: %s", e)
    agrios_router = None  # type: ignore
...
async def enhanced_health() -> Dict[str, Any]:
    ...
    if ENHANCED_BACKEND_AVAILABLE:
        ...
        try:
            ...
            health_data["components"]["tensorflow_serving"] = {
                "status": "error",
                "error": str(e),
            }
...
def dashboard_summary(
    ...
    try:
        cache_path = os.getenv(
            "AGRISENSE_WEATHER_CACHE",
            os.path.join(os.path.dirname(__file__), "..", "..", "weather_cache.csv"),
        )
...
def _is_simple_crop_name_query(q: str) -> bool:
    ...
    toks = [
        t
        for t in _normalize_simple(q).split()
        if t
        and t
        not in [
            "crop",
    ...
    try:
        ...
        for idx, answer in enumerate(_chatbot_answers):
            ...
            if "cultivation guide" in answer_lower and crop_normalized in answer_lower:
                ...
                logger.info(
                    f"✅ Found cultivation guide for crop '{crop_name}' at index {idx}, length: {len(answer)} chars"
                )
                return str(answer)
...
def chat_ask(req: ChatRequest) -> ChatResponse:
    ...
    if any(k in ql for k in ["irrigat", "moisture", "fert", "urea", "dap", "mop", "npk"]) or (
        "water" in ql and not normalized_crop
    ):
...
def analyze_weeds(body: ImageUpload) -> Dict[str, Any]:
    ...
    try:
        ...
        if use_smart_detector:
            ...
            result = smart_detector.analyze_image(image_data=body.image_data, crop_type=body.crop_type or "unknown")
        else:
            ...
            result = {
                ...
                "management_recommendations": {"immediate_actions": ["Check system dependencies"]},
            }
...
def comprehensive_vlm_analysis(body: ImageUpload) -> Dict[str, Any]:
    ...
    try:
        ...
        combined_result = {
            ...
            "confidence_score": (
                disease_analysis.get("confidence_score", 0.5) + weed_analysis.get("confidence_score", 0.5)
            )
            / 2,
        }
        ...
        combined_result["overall_health_score"] = max(
            0,
            min(
                100,
                85
                - (len(disease_recs.get("immediate_actions", [])) * 10)
    ...
    try:
        if VLM_AVAILABLE:
            ...
            return {
                ...
                "models": {
                    "vision_model": (
                        "available" if hasattr(engine, "vision_model") and engine.vision_model else "unavailable"
                    ),
                    "text_encoder": (
                        "available" if hasattr(engine, "text_encoder") and engine.text_encoder else "unavailable"
                    ),
                    "image_processor": (
                        "available" if hasattr(engine, "image_processor") and engine.image_processor else "unavailable"
                    ),
                },
                "version": "1.0.0",
            }
...
def comprehensive_health_assessment(body: ImageUpload) -> Dict[str, Any]:
    ...
    if not PLANT_HEALTH_AVAILABLE:
        raise HTTPException(
            status_code=503,
            detail="Plant health monitoring not available. Install required dependencies.",
        )

    if not plant_health_monitor:
        raise HTTPException(
            status_code=503,
            detail="Plant health monitor not initialized. Check system configuration.",
        )
...
def get_health_system_status() -> Dict[str, Any]:
    ...
    if PLANT_HEALTH_AVAILABLE:
        try:
            ...
            status["model_info"] = {
                "disease_model": disease_info,
                "weed_model": weed_info,
            }
...
def arduino_status() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            rows = cursor.fetchall()
            for row in rows:
                recent_readings.append({"zone_id": row[0], "temperature": row[1], "timestamp": row[2]})
            conn.close()
        except Exception as e:
...
if os.path.isdir(FRONTEND_STATIC_UI):
    ...
    app.mount(
        "/ui",
        StaticFilesWithCache(directory=FRONTEND_STATIC_UI, html=True),
        name="frontend",
    )
...
def chatbot_ask(q: ChatbotQuery) -> Dict[str, Any]:
    ...
    # AGRI-OS enhanced pipeline — try Governor+VRAG before standard retrieval
    try:
        from rag_adapter import enhanced_chatbot_answer
        import asyncio

        enhanced = asyncio.run(
            enhanced_chatbot_answer(
        ...
        pass  # Fall through to standard chatbot
    ...
    if normalized_crop and normalized_crop in SUPPORTED_CROPS:
        if _is_simple_crop_name_query(original_question):
            ...
            if guide:
                return {
                    "question": original_question,
                    "results": [{"rank": 1, "score": 1.0, "answer": guide}],
                }
            else:
                # Return just the crop name as fallback
                return {
                    "question": original_question,
                    "results": [{"rank": 1, "score": 1.0, "answer": normalized_crop}],
                }
    ...
    if not results or (results and results[0].get("score", 0) < 0.25):
        ...
        if was_expanded and results:
            ...
            results[0][
                "answer"
            ] = f"I noticed you asked a short question. Let me help!\n\n{results[0].get('answer', '')}\n\n---\n\n{fallback_answer}"
        else:
...
def chatbot_ask_get(
    ...
    return chatbot_ask(ChatbotQuery(question=question, top_k=top_k, session_id=session_id, language=language))
...
# -------------------------------------------------------------------------
# Crop Encyclopedia & Plants API (Powered by india_crop_dataset.csv)
# -------------------------------------------------------------------------
...
def get_crops() -> Dict[str, Any]:
    """
    Return a rich list of crops from the knowledge base (CSV).
    Used for the 'Crops' page encyclopedia.
    """
    results = []
    try:
        csv_path = Path(__file__).resolve().parent / "india_crop_dataset.csv"
        if not csv_path.exists():
            # Fallback for dev if file missing
            return {"items": [], "warning": "Dataset not found"}

        with open(csv_path, "r", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for i, row in enumerate(reader):
                # Map CSV columns to Frontend CropCard interface
                # Crop,Crop_Category,Soil_Type,pH_Min,pH_Max,Water_Requirement_mm,Growing_Season,Growth_Duration_days

                # Determine urgency/water level
                water_mm = float(row.get("Water_Requirement_mm", 0) or 0)
                water_req = "Medium"
                if water_mm < 350:
                    water_req = "Low"
                elif water_mm > 700:
                ...
                desc = (
                    f"{row['Crop']} is a {row.get('Crop_Category', 'crop')} best grown in {row.get('Soil_Type', 'loam')} soil. "
                    f"It requires {row.get('Water_Requirement_mm')}mm rainfall and a {row.get('Growing_Season', 'season')} climate. "
                    f"Expected yield is around {row.get('Expected_Yield_tonnes_ha')} tonnes/ha."
                )
                ...
                results.append(
                    {
                        ...
                        "name": row["Crop"],
                        "scientificName": f"{row['Crop']} sp.",  # Placeholder
                        "category": row.get("Crop_Category", "General"),
                        "description": desc,
                        "image": f"/images/crops/{row['Crop'].lower()}.jpg",  # Frontend checks if this exists or shows placeholder
                        "waterRequirement": water_req,
                        "season": row.get("Growing_Season", "Year Round"),
                        "tempRange": f"{row.get('Temperature_Min_C')}-{row.get('Temperature_Max_C')}°C",
                        "phRange": f"{row.get('pH_Min')}-{row.get('pH_Max')}",
                        "growthPeriod": f"{row.get('Growth_Duration_days')} days",
                        "tips": [
                            f"Best planted in {row.get('Planting_Month', 'season')}",
                            f"Harvest in {row.get('Harvesting_Month', 'season')}",
                            f"Ideal Moisture: {row.get('Moisture_Optimal_percent')}%",
                        ],
                    }
                )
        return {"items": results}
    except Exception as e:
        logger.exception("Failed to load crops")
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/api/plants")
def get_plants_list() -> Dict[str, Any]:
    """
    Return a simple list of plants for dropdowns (Select Crop Type).
    """
    # Reuse the logic or verify simpler list
    try:
        csv_path = Path(__file__).resolve().parent / "india_crop_dataset.csv"
        if not csv_path.exists():
        ...
        with open(csv_path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                name = row["Crop"]
                if name not in seen:
                    items.append({"id": name.lower(), "name": name})
                    seen.add(name)
        ...
        logger.error(f"Error serving plants list: {e}")
        return {"items": []}

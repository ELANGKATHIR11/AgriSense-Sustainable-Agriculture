...
class AgriSenseMQTTBridge:
    ...
    def on_mqtt_connect(self, client, userdata, flags, rc):
        ...
        if rc == 0:
            ...
            topics = ["agrisense/sensors/data", "agrisense/devices/heartbeat", "agrisense/pump/status"]
            for topic in topics:
    ...
    def handle_sensor_data(self, data: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(
                f"Processed sensor data from {device_id}: "
    ...
    def forward_to_backend(self, reading: SensorReading):
        ...
        try:
            ...
            response = requests.post(f"{self.backend_url}/edge/ingest", json=payload, headers=headers, timeout=10)
    ...
    def get_latest_data(self, device_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if device_id:
            ...
            return {device_id: asdict(reading) for device_id, reading in self.latest_sensor_data.items()}

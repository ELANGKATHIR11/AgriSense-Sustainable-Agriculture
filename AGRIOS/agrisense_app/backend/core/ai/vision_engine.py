...
class CropVisionAnalyst:
    ...
    def __init__(
        ...
        if not TORCH_AVAILABLE:
            raise ImportError(
                "PyTorch and Transformers not installed. " "Install with: pip install -r requirements-ai.txt"
            )
    ...
    def analyze_batch(
        ...
        for i in range(0, len(image_bytes_list), batch_size):
            ...
            logger.info(
                f"Processing batch {i // batch_size + 1}/{(len(image_bytes_list) + batch_size - 1) // batch_size}"
            )
...
def get_crop_vision_analyst() -> CropVisionAnalyst:
    ...
    if _crop_vision_analyst_instance is None:
        ...
        model_name = os.getenv("AGRISENSE_VLM_MODEL", "llava-hf/llava-v1.6-mistral-7b-hf")

...
def log_ml_prediction(
    model_name: str, input_data: Dict[str, Any], prediction: Any, confidence: float, inference_time_ms: float
):
...
def log_sensor_reading(device_id: str, sensor_type: str, value: float, unit: str):
    """Log sensor reading"""
    logger = get_logger("agrisense.sensors")
    logger.debug(
        f"Sensor reading: {device_id}",
        extra={"custom_fields": {"device_id": device_id, "sensor_type": sensor_type, "value": value, "unit": unit}},
    )
...
def log_error(component: str, error_type: str, error_message: str, context: Optional[Dict[str, Any]] = None):
    """Log error with context"""
    logger = get_logger(f"agrisense.{component}")
    logger.error(
        f"{error_type}: {error_message}",
        extra={"custom_fields": {"component": component, "error_type": error_type, "context": context or {}}},
    )


def log_security_event(
    event_type: str, user: Optional[str], action: str, result: str, details: Optional[Dict[str, Any]] = None
):

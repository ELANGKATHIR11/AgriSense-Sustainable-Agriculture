...
class ExplainableAI:
    ...
    async def explain_crop_recommendation(
        self, model: Any, input_features: Dict[str, float], prediction: str, confidence: float
    ) -> Dict[str, Any]:
        """
    ...
    def _generate_friendly_crop_explanation(self, features: Dict[str, float], crop: str) -> str:
        """
    ...
    async def explain_irrigation_decision(
        self, soil_moisture: float, temperature: float, humidity: float, recommendation: str
    ) -> Dict[str, Any]:
        """
        ...
        """
        explanation = {"recommendation": recommendation, "reasons": [], "friendly_explanation": ""}

        # Build reasons list
        if soil_moisture < 30:
            explanation["reasons"].append(
                {"factor": "Low Soil Moisture", "value": f"{soil_moisture}%", "impact": "High priority - soil is dry"}
            )
        ...
        if humidity < 40:
            explanation["reasons"].append(
                {"factor": "Low Humidity", "value": f"{humidity}%", "impact": "Dry air increases water loss"}
            )
    ...
    async def explain_pest_risk(
        self, temperature: float, humidity: float, rainfall: float, risk_level: str, risk_score: float
    ) -> Dict[str, Any]:
        """
        ...
        if humidity > 80:
            explanation["contributing_factors"].append(
                {"factor": "High Humidity", "value": f"{humidity}%", "impact": "Humid conditions favor fungal diseases"}
            )
            explanation["prevention_tips"].append("Improve air circulation by spacing plants properly")
        ...
        if 25 < temperature < 35:
            ...
            explanation["prevention_tips"].append("Monitor plants daily for early signs of pests")
        ...
        if rainfall > 50:
            ...
            explanation["prevention_tips"].append("Avoid overhead irrigation, water at soil level")

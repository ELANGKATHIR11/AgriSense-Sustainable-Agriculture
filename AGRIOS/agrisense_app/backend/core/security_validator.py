...
def detect_sensor_spoofing(reading: Optional[SensorReading] = None, **kwargs):
    ...
    if isinstance(reading, SensorReading):
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Sensor spoofing detected: Invalid sensor value types"
            )
...
class RateLimiter:
    ...
    def is_allowed(self, client_id: str) -> bool:
        ...
        with self.lock:
            ...
            self.tokens[client_id] = min(self.burst_size, self.tokens[client_id] + time_passed * self.rate)
...
async def check_rate_limit(request: Request, limiter: RateLimiter = _api_rate_limiter):
    """
...
def validate_ml_input(features: Dict[str, float]) -> Dict[str, float]:
    ...
    for feature in required_features:
        if feature not in features:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Missing required feature: {feature}")
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid input features: {e}")
...
def sanitize_text_input(text: str, max_length: int = 1000) -> str:
    ...
    if len(text) > max_length:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Input too long (max {max_length} characters)"
        )

    # Remove potential injection attempts (sanitize rather than reject)
    dangerous_patterns = ["<script", "javascript:", "onerror=", "onload=", "<?php", "<?", "<%", "${", "exec(", "eval("]
...
class DeviceCertificateValidator:
    ...
    def validate_device_certificate(self, device_id: str, certificate: Optional[str] = None) -> bool:
        """

...
class HealthCheckResult:
    ...
    def __init__(self, status: ServiceStatus, message: str = "", details: Optional[Dict[str, Any]] = None):
        self.status = status
...
def with_fallback(fallback_func: Callable, log_errors: bool = True, raise_on_fallback: bool = False):
    """
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        def wrapper(*args, **kwargs) -> T:
            try:
                ...
                if log_errors:
                    logger.warning(
                        f"Primary function {func.__name__} failed: {e}, " f"using fallback {fallback_func.__name__}"
                    )
...
class MLFallbackManager:
    ...
    def get_circuit_breaker(self, model_name: str) -> CircuitBreaker:
        ...
        if model_name not in self.circuit_breakers:
            self.circuit_breakers[model_name] = CircuitBreaker(failure_threshold=3, timeout=120.0, name=model_name)
        return self.circuit_breakers[model_name]

    def predict_with_fallback(
        self, model_name: str, ml_predict_func: Callable, rule_based_func: Callable, input_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    async def run_all_checks(self) -> Dict[str, HealthCheckResult]:
        ...
        for name, check_func in self._checks.items():
            try:
                ...
                results[name] = HealthCheckResult(status=ServiceStatus.UNHEALTHY, message=f"Check failed: {str(e)}")
...
async def check_database_health() -> HealthCheckResult:
    ...
    try:
        ...
        return HealthCheckResult(
            status=ServiceStatus.HEALTHY, message="Database connected", details={"connection": "active"}
        )
    except Exception as e:
        return HealthCheckResult(status=ServiceStatus.UNHEALTHY, message=f"Database error: {e}")
...
async def check_cache_health() -> HealthCheckResult:
    ...
    try:
        ...
        if stats.get("connected"):
            return HealthCheckResult(status=ServiceStatus.HEALTHY, message="Cache connected", details=stats)
        else:
            return HealthCheckResult(
                status=ServiceStatus.DEGRADED, message="Cache unavailable, using fallback", details=stats
            )
    except Exception as e:
        return HealthCheckResult(status=ServiceStatus.DEGRADED, message=f"Cache check failed: {e}")
...
async def check_ml_models_health() -> HealthCheckResult:
    ...
    try:
        ...
        return HealthCheckResult(
            status=ServiceStatus.HEALTHY, message=f"{model_count} models loaded", details={"loaded_models": model_count}
        )
    except Exception as e:
        return HealthCheckResult(status=ServiceStatus.DEGRADED, message=f"ML models check failed: {e}")

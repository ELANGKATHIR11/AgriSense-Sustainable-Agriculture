...
class AlertManager:
    ...
    async def send_alert(
        ...
        logger.info(
            ...
            extra={"custom_fields": {"alert_id": alert.alert_id, "device_id": device_id, "level": level.value}},
        )
        ...
        try:
            ...
            for phone in phone_numbers:
                message = self.twilio_client.messages.create(
                    body=sms_body[:160], from_=settings.twilio_phone_number, to=phone  # SMS character limit
                )
    ...
    async def _send_whatsapp(self, alert: Alert, recipients: Optional[List[str]]):
        ...
        try:
            ...
            for phone in phone_numbers:
                message = self.twilio_client.messages.create(
                    body=wa_body, from_=f"whatsapp:{settings.twilio_phone_number}", to=f"whatsapp:{phone}"
                )

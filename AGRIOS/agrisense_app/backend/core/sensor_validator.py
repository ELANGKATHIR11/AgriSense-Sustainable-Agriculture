...
class SensorValidator:
    ...
    def validate_reading(self, device_id: str, data: Dict[str, Any], strict: bool = True) -> Dict[str, Any]:
        """
        ...
        if "temperature" in data:
            ...
            if not isinstance(temp, (int, float)):
                ...
                errors.append(
                    f"Temperature {temp}°C out of range " f"[{settings.sensor_min_temp}, {settings.sensor_max_temp}]"
                )
        ...
        if device_id in self.last_readings:
            ...
            if last_time and "temperature" in data and "temperature" in last_data["data"]:
                ...
                if time_diff < 60:  # Within 1 minute
                    ...
                    if temp_diff > 10.0:
                        warnings.append(f"Suspicious temperature change: {temp_diff}°C in {time_diff}s")

        # Store this reading for next comparison
        self.last_readings[device_id] = {"data": data.copy(), "timestamp": datetime.now()}
        ...
        if errors:
            ...
            if self.anomaly_counts[device_id] >= 3:
                logger.error(
                    f"Multiple validation failures for device {device_id}. " f"Possible tampering! Errors: {errors}"
                )

            if strict:
                raise SensorTamperingError(f"Sensor validation failed for {device_id}: {', '.join(errors)}")

        return {"valid": len(errors) == 0, "data": data, "warnings": warnings, "errors": errors}

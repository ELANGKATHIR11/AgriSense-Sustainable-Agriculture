...
def get_weather_data() -> Dict[str, Any]:
    ...
    return {"temperature_c": 28.0, "humidity_pct": 65.0, "rainfall_mm": 0.0, "wind_speed_kmh": 5.0, "et0_mm": 4.5}
...
def get_recommendation_stats() -> Dict[str, Any]:
    ...
    conn = get_conn()
    cur = conn.execute(
        "SELECT COUNT(*) as total, AVG(water_liters) as avg_water, AVG(expected_savings_liters) as avg_savings FROM reco_history"
    )
    row = cur.fetchone()
    conn.close()
    if row:
...
def get_sensor_stats() -> Dict[str, Any]:
    ...
    conn = get_conn()
    cur = conn.execute(
        "SELECT COUNT(*) as total, COUNT(DISTINCT zone_id) as zones, AVG(moisture_pct) as avg_moisture FROM readings"
    )
    row = cur.fetchone()
    conn.close()
    if row:

...
def decode_token(token: str) -> TokenData:
    ...
    try:
        ...
        return TokenData(username=username, user_id=user_id, role=role, exp=datetime.fromtimestamp(exp))
    except JWTError:
        raise credentials_exception
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    return Token(
        access_token=access_token, token_type="bearer", expires_in=JWT_EXP_MINUTES * 60, refresh_token=refresh_token
    )
...
async def refresh_access_token(refresh_token: str = Form(...)):
    ...
    try:
        ...
        return Token(access_token=access_token, token_type="bearer", expires_in=JWT_EXP_MINUTES * 60)
...
if __name__ == "__main__":
    ...
    async def test_auth():
        ...
        token = create_access_token(
            {"sub": authenticated.username, "user_id": authenticated.user_id, "role": authenticated.role}
        )

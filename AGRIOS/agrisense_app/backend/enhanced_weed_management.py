...
class WeedSegmentationModel:
    ...
    def _load_pretrained_model(self):
        ...
        try:
            if getattr(transformers, "AutoImageProcessor", None) and getattr(
                transformers, "AutoModelForSemanticSegmentation", None
            ):
                ...
                if selected is None:
                    ...
                    logger.warning(
                        f"No compatible HF semantic segmentation model loaded (last error: {selected_error}); falling back to PyTorch DeepLabV3"
                    )
                ...
                self.model = segmentation.deeplabv3_resnet50(  # type: ignore
                    pretrained=True, num_classes=21  # COCO classes
                )
                self.model.to(self.device)
                self.model.eval()
    ...
    def segment_weeds(self, image: np.ndarray) -> Dict[str, Any]:
        ...
        if not self.model_loaded:
            return {"success": False, "error": "Model not loaded", "segments": [], "weed_coverage": 0.0}
        ...
        try:
            ...
            return {"success": False, "error": str(e), "segments": [], "weed_coverage": 0.0}
    ...
    def classify_weed(self, image: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            if not TORCH_AVAILABLE or not torch or self.model is None or self.transform is None:
                return {"success": False, "error": "Model or transform not available", "predictions": []}
            ...
            if TORCH_AVAILABLE and torch and hasattr(transformed, "unsqueeze") and hasattr(transformed, "to"):
                ...
                return {"success": False, "error": "Transform did not return tensor", "predictions": []}
                ...
                with torch.no_grad():
                    ...
                    if hasattr(torch, "nn") and hasattr(torch.nn, "functional") and F is not None:
                        ...
                        try:
                            ...
                            if hasattr(out0, "dim") or (
                                TORCH_AVAILABLE
                                and torch is not None
                                and hasattr(torch, "is_tensor")
                                and torch.is_tensor(out0)
                            ):
                        ...
                        return {"success": False, "error": "torch.nn.functional not available", "predictions": []}
            else:
                return {"success": False, "error": "torch.no_grad not available", "predictions": []}

            # Get top predictions
            if hasattr(torch, "topk"):
                top_prob, top_class = torch.topk(probabilities, min(5, len(self.class_names)))  # type: ignore
            else:
                return {"success": False, "error": "torch.topk not available", "predictions": []}
        ...
        try:
            ...
            for weed_name in detected_weeds:
                if weed_name in self.weed_database:
                    ...
                    # Cultural controls
                    recommendations["cultural_controls"].extend(weed_info.get("cultural_controls", []))

                    # Timing recommendations
                    recommendations["timing_recommendations"].append(
                        {"weed": weed_name, "optimal_timing": weed_info.get("timing", "consult_expert")}
                    )
    ...
    def get_treatment_plan(self, field_conditions: Dict[str, Any], detected_weeds: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            pre_emergent_weeds = [
                ...
                if weed in self.weed_database and "pre-emergent" in self.weed_database[weed].get("herbicides", [])
            ]
            ...
            post_emergent_weeds = [weed for weed in detected_weeds if weed in self.weed_database]

...
class SmartWeedDetector:
    ...
    def _analyze_patterns(self, image_array: np.ndarray) -> Dict[str, float]:
        ...
        return {"edge_regularity": float(edge_regularity), "pattern_regularity": float(pattern_regularity)}
    ...
    def _analyze_organization(self, image_array: np.ndarray) -> Dict[str, float]:
        ...
        return {"organization_score": float(organization_score), "uniformity": float(uniformity)}
    ...
    def _generate_response(
        self, classification: Dict[str, Any], analysis_result: Dict[str, Any], crop_type: str
    ) -> Dict[str, Any]:
            ...
            base_response.update(
                {
                    "analysis_summary": {
                        ...
                        "crop_health": (
                            "Good" if analysis_result["green_health"]["green_intensity"] > 0.6 else "Moderate"
                        ),
                        "organization": (
                            "Well Organized"
                            if analysis_result["organization_stats"]["organization_score"] > 0.3
                            else "Moderate"
                        ),
            ...
            base_response.update(
                {
                    "analysis_summary": {
                        ...
                        "weed_diversity": (
                            "High" if analysis_result["color_stats"]["color_diversity"] > 0.5 else "Moderate"
                        ),
                    ...
                    "management_recommendations": {
                        ...
                        "timing": (
                            ...
                            else "Plan treatment within 1-2 weeks"
                        ),
                    },
                }
            )
    ...
    def _generate_error_response(self, error_message: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "management_recommendations": {"immediate_actions": ["Please try uploading the image again"]},
        }

...
def _augment_with_governor(
    base_result: Dict[str, Any],
    crop_type: str = "unknown",
) -> Dict[str, Any]:
    """Attach AGRI-OS Decision Governor recommendation when available."""
    try:
        from agrios.decision_governor import DecisionGovernor
        from agrios.schemas import VisionResult

        gov = DecisionGovernor()
        confidence = float(base_result.get("detection_confidence", base_result.get("confidence", 0.0)))
        # Normalize confidence to 0-1 range (some models return percentage 0-100)
        if confidence > 1.0:
            confidence = confidence / 100.0
        confidence = min(max(confidence, 0.0), 1.0)
        # Weed systems use different keys sometimes, normalize to 'disease' logic for governor
        weed_name = base_result.get("weed_type", base_result.get("dominant_weed_species", "weed_spp"))

        vision = VisionResult(
            disease_name=weed_name,  # reusing disease field for weed name
            confidence=confidence,
            severity=base_result.get("weed_pressure", "unknown"),
        )

        inputs = {
            "vision_result": vision,
            "crop_context": {"crop_type": crop_type, "disease": weed_name},
        }

        decision = gov.decide(inputs)
        enriched = dict(base_result)
        enriched["governor_decision"] = decision if isinstance(decision, dict) else decision.model_dump()
        return enriched
    except Exception as exc:
        logger.debug("Governor augmentation skipped: %s", exc)
        return base_result
...
class WeedManagementEngine:
    ...
    def detect_weeds(
        ...
        try:
            ...
            if scold_result.get("success") and scold_result.get("detections"):
                ...
                return _augment_with_governor(scold_result, crop_type)
        ...
        if ENHANCED_AVAILABLE:
            try:
                ...
                if enhanced_result.get("success", True):
                    ...
                    base = _augment_with_vlm(image_data, crop_type, base)
                    return _augment_with_governor(base, crop_type)
            except Exception as e:
    ...
    def _format_enhanced_result(
        self, enhanced_result: Dict[str, Any], crop_type: str, environmental_data: Optional[Dict] = None
    ) -> Dict[str, Any]:
                    ...
                    formatted_result["detected_weeds"].append(
                        {
                            ...
                            "area_percent": segment["area"]
                            / (enhanced_result["image_info"]["width"] * enhanced_result["image_info"]["height"])
                            * 100,
                        }
                    )
        ...
        try:
            ...
            base = {
                ...
                "model_info": {
                    ...
                    "accuracy": (round(self.model_accuracy * 100, 1) if self.model_accuracy is not None else None),
                },
            }
            return base
    ...
    def generate_management_plan(self, detected_weeds: List[str], field_conditions: Dict[str, Any]) -> Dict[str, Any]:
        ...
        mock_segmentation = {
            "weed_regions": [{"weed_type": weed, "coverage": 0.1, "density": "moderate"} for weed in detected_weeds],
...
def get_weed_management_recommendations(
    detected_weeds: List[str], field_conditions: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:

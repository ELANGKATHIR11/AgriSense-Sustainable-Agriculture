...
def _is_scold_available() -> bool:
    ...
    try:
        ...
        response = requests.get(f"{base_url}/health", timeout=timeout / 2)
...
def detect_disease_with_scold(
    image_data: Any, crop_type: str = "unknown", timeout_s: float = None
    ...
    try:
        ...
        recommendations = _get_disease_treatment_recommendations([d["name"] for d in diseases], crop_type)
...
def detect_weeds_with_scold(
    ...
    try:
        ...
        treatment_options = _get_weed_treatment_recommendations([w["name"] for w in weeds], severity, crop_type)
...
def _get_disease_treatment_recommendations(diseases: List[str], crop_type: str = "unknown") -> List[str]:
    ...
    disease_treatments = {
        ...
        "wilt": ["Remove infected plant", "Improve soil drainage", "Rotate crops", "Sanitize equipment"],
    }
...
def _get_weed_treatment_recommendations(
    weeds: List[str], severity: str = "low", crop_type: str = "unknown"
) -> List[str]:
    """Generate treatment recommendations for detected weeds."""

    weed_treatments = {
        "bermuda grass": ["Use glyphosate herbicide", "Manual removal for small areas", "Mulching to prevent growth"],
        "crabgrass": [
            "Apply pre-emergent herbicide early spring",
            "Post-emergent herbicide in summer",
            "Maintain dense crop coverage",
        ],
        "chickweed": ["Hand pull in moist soil", "Apply selective herbicide", "Use pre-emergent before germination"],
        "dandelion": [
            "Manual removal with root extraction",
            "Targeted herbicide application",
            "Repeated removal to exhaust root reserves",
        ],
        "thistle": ["Cut below soil line regularly", "Apply herbicide to fresh cuts", "Remove before seed production"],
    }

...
def _augment_with_governor(
    ...
    try:
        ...
        disease = base_result.get("disease_type", "unknown")

        vision = VisionResult(
            disease_name=disease,
            confidence=confidence,
            severity=base_result.get("severity", "unknown"),
            bbox=base_result.get("box", None),
        )

        inputs = {
            "vision_result": vision,
            "crop_context": {"crop_type": crop_type, "disease": disease},
        }
...
class DiseaseDetectionEngine:
    ...
    def _load_config(self) -> None:
        try:
            ...
            self.config = {"disease_detection": {"preprocessing": {"resize": [224, 224]}, "confidence_threshold": 0.7}}
    ...
    def _postprocess_predictions(self, outputs: Any, top_k: int = 3) -> List[Dict[str, Any]]:
        ...
        treatments = self.treatment_recommendations.get(
            disease, {"immediate": ["Inspect plants"], "prevention": ["Monitor"]}
        )
        fallback = {
            "timestamp": _get_timestamp(),
    ...
    def detect_disease(
        ...
        **kwargs,
        ...
        The result is enriched with a Decision Governor recommendation when available.
        """
        result: Dict[str, Any] = {}
        ...
        try:
            ...
            if scold_result.get("success") and scold_result.get("detections"):
                ...
                result = scold_result
                return _augment_with_governor(result, crop_type)
        ...
        try:
            ...
            if self.model is not None:
                ...
                if preds:
                    ...
                    result = _augment_with_vlm_result(image_data, crop_type, result)
                    return _augment_with_governor(result, crop_type)
        except Exception:
        ...
        try:
            ...
            try:
                ...
                if isinstance(res, dict):
                    ...
                    result = _augment_with_vlm_result(image_data, crop_type, res)
                    return _augment_with_governor(result, crop_type)
            except Exception:
                logger.info("comprehensive_detector present but failed; falling back")
        except Exception:
...
def analyze_disease_image_enhanced(
    image_data: Union[str, bytes, Image.Image], crop_type: str = "unknown"
) -> Dict[str, Any]:

...
def test_vlm_analysis():
    ...
    try:
        ...
        print(f"  ✅ Disease analysis completed")
        ...
        print(f"  ✅ Weed analysis completed")
...
def test_api_endpoints(base_url="http://localhost:8004"):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"  ✅ VLM Status: {status_data.get('status', 'unknown')}")
    ...
    try:
        ...
        if response.status_code == 200:
            result = response.json()
            print(f"  ✅ Weed analysis successful")
            print(f"     Coverage: {result.get('weed_coverage_percentage', 0):.1f}%")
            print(f"     Pressure: {result.get('weed_pressure', 'unknown')}")
            if 'vlm_analysis' in result:
                print(f"     VLM Enhanced: {result['vlm_analysis'].get('knowledge_matches', 0)} matches")
        else:
            print(f"  ❌ Weed analysis failed: {response.status_code}")
            print(f"     Response: {response.text[:200]}")
    except Exception as e:
        print(f"  ❌ Weed analysis error: {e}")
    
    # Test comprehensive VLM analysis endpoint
    try:
        print("  Testing comprehensive VLM analysis endpoint...")
        response = requests.post(
        ...
        if response.status_code == 200:
            ...
            print(f"     Health Score: {result.get('overall_health_score', 0):.1f}")
...
def main():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("⚠️  Server not responding properly, skipping API tests")
    except Exception as e:
    ...
    if passed == total:
        print("All core VLM tests passed!")

...
async def lifespan(app: FastAPI):
    ...
    # Startup
    try:
        # Initialize PocketDB backend
        logger.info("Initializing PocketDB database...")
        try:
            from agrisense_app.backend.database import init_database, get_database_manager
            
            db_backend = os.getenv("AGRISENSE_DB_BACKEND", "sqlite").lower()
            app.state.db = await init_database(db_backend)
            logger.info(f"✓ Database initialized: {db_backend}")
            
            # Log database statistics
            stats = await app.state.db.get_stats()
            logger.info(f"Database collections: {list(stats.get('collections', {}).keys())}")
        except Exception as db_err:
            logger.warning(f"Could not initialize PocketDB backend: {db_err}")
            logger.info("Continuing with legacy data store")
        
        # Initialize ML system (Inference Engine & RAG Pipeline)
        try:
            ...
            logger.info("✅ ML Inference Engine loaded with 6 trained models")
            
            logger.info("🔮 Initializing RAG Pipeline...")
            pipeline = initialize_rag_pipeline()
            logger.info("✅ RAG Pipeline ready (Intent Classification + Semantic Retrieval)")
            
            app.state.ml_engine = engine
            app.state.rag_pipeline = pipeline
        except Exception as ml_err:
            logger.warning(f"⚠️  ML system unavailable: {ml_err}")
            logger.info("App will continue with standard features only")
...
try:
    # Parse CORS origins from environment or use common local dev defaults
    # Include React/Vite dev ports and the Firebase hosting domain used for preview
    _cors_origins_env = os.getenv(
        "CORS_ORIGINS",
        "http://localhost:3000,http://127.0.0.1:3000,http://localhost:5173,http://127.0.0.1:5173,http://localhost:8080,http://127.0.0.1:8080,http://localhost:8081,http://127.0.0.1:8081,https://agrisense-fe79c.web.app",
    )
...
# Include RAG & Inference routes (Hybrid RAG Pipeline)
try:
    if __package__:
        from .api.ml_routes import mount_ml_routes
    else:
        from api.ml_routes import mount_ml_routes  # type: ignore
    
    mount_ml_routes(app)
    logger.info("✅ RAG & Inference routes mounted (/api/v1/ml/*)")
except Exception as _rag_exc:
    logger.warning("Could not include RAG routes: %s", _rag_exc)
...
# CORS: allow development origins by default; allow configuring specific origins via env
_origins_env = os.getenv(
    "ALLOWED_ORIGINS",
    "http://localhost:3000,http://127.0.0.1:3000,http://localhost:5173,http://127.0.0.1:5173,http://localhost:8080,http://127.0.0.1:8080,http://localhost:8081,http://127.0.0.1:8081,https://agrisense-fe79c.web.app",
)
_allow_origins = [o.strip() for o in _origins_env.split(",") if o.strip()] or ["http://localhost:3000", "http://localhost:5173", "http://localhost:8081", "http://127.0.0.1:8081"]
app.add_middleware(
    CORSMiddleware,
...
async def health_database() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "connected": app.state.db._is_initialized,
...
def _dataset_to_cards() -> List[CropCard]:
    ...
    try:
        for row in rows:
            # Support both old (Crop) and new (crop_name) schemas
            name = str((row.get("crop_name") or row.get("Crop") or row.get("crop") or "")).strip()
            if not name:
                continue
            
            scientific_name = str((row.get("scientific_name") or "")).strip() or None
            
            cat = str((row.get("crop_type") or row.get("Crop_Category") or row.get("category") or "")).strip() or None
            
            try:
                ph_min = row.get("min_pH") or row.get("pH_Min") or row.get("ph_min")
                ph_max = row.get("max_pH") or row.get("pH_Max") or row.get("ph_max")
                ph_range = f"{float(ph_min):.1f}-{float(ph_max):.1f}" if ph_min and ph_max else None
            except Exception:
                ph_range = None
                
            try:
                t_min = row.get("min_temp_C") or row.get("Temperature_Min_C") or row.get("temperature_min_c")
                t_max = row.get("max_temp_C") or row.get("Temperature_Max_C") or row.get("temperature_max_c")
                temp_range = f"{int(float(t_min))}-{int(float(t_max))}°C" if t_min and t_max else None
            except Exception:
                temp_range = None
                
            try:
                growth_days = row.get("growth_duration_days") or row.get("Growth_Duration_days") or row.get("growth_days")
                growth_period = f"{int(float(growth_days))} days" if growth_days else None
            except Exception:
                growth_period = None
                
            try:
                # New schema: water_req_mm_day * growth_duration_days
                w_mm_day = row.get("water_req_mm_day")
                if w_mm_day and growth_days:
                     total_water = float(w_mm_day) * float(growth_days)
                     water_req = _bucket_water_req(total_water)
                else:
                    # Old schema
            ...
            season = str((row.get("season") or row.get("Growing_Season") or "")).strip() or None
...
# List of all 100 supported crops (comprehensive Indian agriculture dataset)
SUPPORTED_CROPS = [

...
def load_or_init_tabpfn(task: str):
    ...
    try:
        ...
        # Fallback dummy seed
        X = np.random.rand(50, 7) if task == "crop_recommendation" else np.random.rand(50, 3)
        y = np.random.choice(["Rice", "Maize", "Beans"], 50) if task == "crop_recommendation" else np.random.choice([0, 1], 50)
        model.fit(X, y)
...
async def predict_tabular(payload: TabularPredictInput):
    task = payload.task
    if task not in ["crop_recommendation", "fertilizer_recommendation", "irrigation_optimization"]:
    ...
    try:
        if task == "crop_recommendation":
            x = np.array([feats['N'], feats['P'], feats['K'], feats['temperature'], feats['humidity'], feats['ph'], feats['rainfall']]).reshape(1, -1)
            probs = model.predict_proba(x)[0]
            classes = model.classes_
            top_indices = np.argsort(probs)[::-1][:3]
            crops = [{"name": str(classes[idx]), "suitability": float(round(probs[idx] * 100, 1))} for idx in top_indices]
            return {
                "crops": crops,
                "optimalPH": "Healthy neutral optimal pH zone" if 6.0 <= feats['ph'] <= 7.0 else "Sub-optimal pH",
                "nutritionStatus": f"NPK ratios balanced for horticulture."
            }
        elif task == "fertilizer_recommendation":
            x = np.array([feats['temperature'], feats['humidity'], feats['moisture'], feats['N'], feats['K'], feats['P']]).reshape(1, -1)
            ...
            x = np.array([feats['moisture'], feats['temperature'], feats['humidity']]).reshape(1, -1)
            prob = model.predict_proba(x)[0]
            ...
            return {
                ...
                "moistureStatus": "CRITICAL UNDERWATERED" if feats['moisture'] < 20 else "NORMAL" if action == 0 else "MODERATE MOISTURE STRESS",
                "advice": "Irrigation sequence optimized by TabPFN tabular engine." if water_liters > 0 else "No watering sequence needed.",
                "durationMinutes": int(water_liters / 40) if water_liters > 0 else 0,
                "irrigationSchedule": "Daily dawn interval" if water_liters > 0 else "Standby"
            }
    except Exception as e:
...
async def status_tabular():
    ...
    return {
        ...
        "vram_usage": "CUDA Available" if torch.cuda.is_available() else "None (CPU Mode)"
    }

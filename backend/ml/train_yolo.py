...
try:
    ...
    logger.warning("ultralytics package not installed globally. Using mock trainer interface for verification.")
    YOLO = None

def run_yolo_training(data_yaml_path: str = "train.yaml", epochs: int = 50, batch_size: int = -1):
    """
    ...
    try:
        ...
        best_model_path = os.path.join("agrisense_yolo", "train_run", "weights", "best.pt")
        ...
        if "out of memory" in str(e).lower() or "cuda" in str(e).lower():
            logger.warning("CUDA Out of Memory or GPU issue. Falling back to lightweight YOLOv11s model...")
            try:
                ...
                best_model_path = os.path.join("agrisense_yolo_fallback", "train_fallback", "weights", "best.pt")
                _export_model(model)
                return best_model_path

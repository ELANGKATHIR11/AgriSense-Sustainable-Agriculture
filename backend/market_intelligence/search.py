...
async def search_duckduckgo_package(query: str, max_results: int = 10) -> list[str]:
    ...
    try:
        ...
        logger.warning(
            f"duckduckgo-search package execution failed: {e}. Falling back to HTML."
        )
        return []
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=12.0, follow_redirects=True) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning("DDG HTML endpoint rate limited (429). Retrying...")
                else:
                    logger.warning(
                        f"DDG HTML returned {response.status_code}. Retrying..."
                    )
        except Exception as e:
...
async def search_duckduckgo(query: str, max_results: int = 10) -> list[str]:
    ...
    ranked_links = sorted(
        filtered_links, key=lambda x: get_source_confidence(x), reverse=True
    )

...
def compute_freshness_score(timestamp: datetime) -> float:
    ...
    if not timestamp:
        return 0.1
    elapsed = datetime.utcnow() - timestamp.replace(tzinfo=None)
...
def compute_analytics(crop: str, db: Session) -> dict:
    ...
    prices_30d = db.query(MarketPrice).filter(
        MarketPrice.crop.ilike(crop),
        MarketPrice.timestamp >= thirty_days_ago
    ).all()
    ...
    # Moving Averages
    prices_7d = [p.price for p in prices_30d if p.timestamp >= (now - timedelta(days=7))]
    ...
    def get_avg_at_period(start_date, end_date):
        res = db.query(func.avg(MarketPrice.price)).filter(
            MarketPrice.crop.ilike(crop),
    ...
    avg_last_month = get_avg_at_period(now - timedelta(days=31), now - timedelta(days=30))
    
    daily_change = ((avg_today - avg_yesterday) / avg_yesterday * 100) if avg_yesterday else 0.0
    weekly_change = ((avg_today - avg_last_week) / avg_last_week * 100) if avg_last_week else 0.0
    monthly_change = ((avg_today - avg_last_month) / avg_last_month * 100) if avg_last_month else 0.0
...
def normalize_and_save_prices(db: Session, records: list[dict], url: str):
    ...
    for r in records:
        ...
        existing_list = db.query(MarketPrice).filter(
        ...
        calculated_confidence = (initial_confidence * 0.4) + (source_rank / 100.0 * 0.4) + (min(verification_count, 5) / 5.0 * 0.2)

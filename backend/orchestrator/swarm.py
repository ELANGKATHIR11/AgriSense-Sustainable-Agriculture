...
class SwarmOrchestrator:
    ...
    def get_agent_by_role(self, role_keyword: str):
        for name, agent in self.agents.items():
            if role_keyword.lower() in agent.role.lower() or role_keyword.lower() in name.lower():
                return agent
        return None
    ...
    async def execute_swarm_pipeline(self, task: str) -> Dict[str, Any]:
        ...
        """
        pipeline_id = f"flow-{int(datetime.utcnow().timestamp())}"
        logger.info(f"Starting Swarm Pipeline {pipeline_id} for task: '{task}'")
        
        steps = [
            {"step": "1. Strategic Alignment", "agent_role": "CEOAgent", "desc": "Assessing alignment, planning scope, and mapping risks."},
            {"step": "2. High-Level Planning", "agent_role": "PlannerAgent", "desc": "Drafting technical architecture blueprints and flowcharts."},
            {"step": "3. Task Decomposition", "agent_role": "TaskDecomposerAgent", "desc": "Splitting the master plan into discrete JIRA-like subtasks."},
            {"step": "4. System Architecture", "agent_role": "ArchitectAgent", "desc": "Defining class boundaries, API schemas, and SQLite models."},
            {"step": "5. Full-Stack Execution", "agent_role": "FullStackAgent", "desc": "Writing clean Python code, endpoints, and React templates."},
            {"step": "6. Quality Assurance", "agent_role": "QAAgent", "desc": "Validating outputs via mock unit tests and edge cases."},
            {"step": "7. Security Scanning", "agent_role": "SecurityReviewAgent", "desc": "Running dependency vulnerability audits and secrets scanning."},
            {"step": "8. Performance Auditing", "agent_role": "PerformanceReviewAgent", "desc": "Benchmarking queries, latency, and resource footprint."},
            {"step": "9. Technical Documentation", "agent_role": "DocumentationAgent", "desc": "Generating OpenAPI schemas, README.md, and changelogs."},
        ]
        ...
        for s in steps:
            ...
            if not agent:
                ...
                logger.warning(f"Agent {s['agent_role']} not found. Initializing virtual placeholder.")
                continue
            ...
            try:
                ...
                # Executing task
                result = await agent.execute_task(f"Context: {current_input}. Task: {s['desc']}")
                
                s_log["status"] = "completed"
                s_log["result"] = result
                s_log["completed_at"] = datetime.utcnow().isoformat() + "Z"
                ...
                if bug_fixer:
                    ...
                    fix_res = await bug_fixer.execute_task(f"Analyze failure: {e} in step {s['step']}. Fix root cause.")
        ...
        if workflow_log["status"] != "failed":
            ...
            workflow_log["completed_at"] = datetime.utcnow().isoformat() + "Z"

        self.execution_history.append(workflow_log)
        # Save complete workflow run to SQLite memory system
        memory_system.log_task("SwarmOrchestrator", task, workflow_log)

        # Remove from active
        self.active_workflows = [w for w in self.active_workflows if w["id"] != pipeline_id]

...
class DigitalTwinPipeline:
    ...
    def bootstrap_eif(self):
        ...
        try:
            if os.path.exists(CLEANED_DATA_PATH):
                ...
                features = df[["Nitrogen", "Phosphorous", "Potassium", "Temparature", "Humidity", "pH", "Moisture"]].values
    ...
    def execute_pipeline(self, telemetry: dict) -> dict:
        ...
        anomaly_alerts = []
        if has_anomaly:
            anomaly_alerts.append(f"Extended Isolation Forest flagged sensor anomaly (score: {anomaly_score:.3f}).")
        ...
        try:
            ...
            suitability_confidence = max(0.1, min(1.0, suitability_confidence + correction))
            predicted_crop = str(tabpfn.classes_[best_idx])
            recommendation_notes.append(f"TabPFN correction recommends: {predicted_crop} (Conf: {suitability_confidence:.1%})")
        except Exception as e:
        ...
        try:
            ...
            x_yield = torch.tensor([[1.0, 100.0, temp, telemetry.get("N", 50.0), telemetry.get("P", 40.0), telemetry.get("K", 40.0)]], dtype=torch.float32)
            with torch.no_grad():
                pred_val = float(yield_model(x_yield).cpu().numpy()[0])
            recommendation_notes.append(f"FT-Transformer projected yield index: {pred_val:.2f} tons/acre")
        except Exception as e:
            recommendation_notes.append(f"FT-Transformer yield prediction bypassed: {str(e)}")

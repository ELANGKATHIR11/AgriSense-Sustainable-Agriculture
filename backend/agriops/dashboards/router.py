...
async def get_agriops_overview(db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to gather AgriOps overview: {str(e)}")
...
async def promote_model_challenger(model_id: str = Body(..., embed=True), db: Session = Depends(get_db)):
    """

...
class LiveSearchService:
    async def search_and_cache_live_knowledge(self, db: Session, query: str) -> List[Dict[str, Any]]:
        """
        ...
        cached_record = db.query(ScrapeCache).filter(ScrapeCache.url.like(f"%query={query}%")).first()
        if cached_record:
        ...
        for idx, art in enumerate(simulated_online_articles):
            ...
            doc_id = f"live-{int(datetime.utcnow().timestamp())}-{idx}"
            ...
            indexed_docs.append({
                ...
                "metadata": {"title": art["title"], "url": art["url"], "source": "live_web_search"}
            })

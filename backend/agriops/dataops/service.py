...
class DataOpsService:
    ...
    async def validate_and_ingest_metrics(self, db: Session, payload: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        missing_fields = 0
        total_fields = 7
        req_fields = ["soil_moisture", "temperature", "humidity", "ph", "nitrogen", "phosphorus", "potassium"]
        ...
        history = db.query(SensorReading).order_by(SensorReading.timestamp.desc()).limit(100).all()
        drift_report = {"drift_detected": False, "msg": "Insufficient history for drift analysis"}
        
        if len(history) >= 10:
            # Prepare arrays for drift analysis
            ref_dict = {
                "soil_moisture": [h.soil_moisture for h in history if h.soil_moisture is not None],
                "temperature": [h.temperature for h in history if h.temperature is not None],
            ...
            try:
                drift_res = self.drift_detector.analyze_dataset_drift(ref_dict, curr_dict)
                drift_report = {
        ...
        # Fire event
        await event_bus.publish("DatasetValidated", {
    ...
    def get_dataset_registry_summary(self, db: Session) -> Dict[str, Any]:
        ...
        latest_drone = db.query(DroneImage).order_by(DroneImage.flight_date.desc()).first()
        latest_satellite = db.query(SatelliteMetadata).order_by(SatelliteMetadata.acquisition_date.desc()).first()
        ...
        if latest_drone:
            lineage.append({
                ...
                "timestamp": latest_drone.flight_date.isoformat() + "Z" if latest_drone.flight_date else None,
        ...
        if latest_satellite:
            lineage.append({
                ...
                "timestamp": latest_satellite.acquisition_date.isoformat() + "Z" if latest_satellite.acquisition_date else None,
                "description": f"Satellite data registry scan. Cloud cover: {latest_satellite.cloud_cover}%"
            })

        return {
            "datasets": [
                {"name": "IoT-Sensor-Telemetry", "type": "Tabular", "count": sensor_readings_count, "format": "Real-time MQTT"},
                {"name": "Sentinel-Satellite-Imagery", "type": "Raster", "count": satellite_scans_count, "format": "TIFF"},
                {"name": "Drone-Survey-Orthomosaics", "type": "Image", "count": drone_images_count, "format": "JPG"}
            ],
            "lineages": lineage,

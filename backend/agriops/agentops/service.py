...
class AgentOpsService:
    ...
    async def run_agent_task(self, db: Session, agent_name: str, task_title: str, input_details: str) -> Dict[str, Any]:
        """
    ...
    def get_agent_swarm_overview(self, db: Session) -> Dict[str, Any]:
        ...
        audits = db.query(AuditLog).filter(AuditLog.action == "AGENT_EXECUTION").limit(30).all()

        total_runs = len(audits)
        success_runs = len([a for a in audits if "success" in getattr(a, "details", "")])
        ...
        return {
            ...
            "success_rate": f"{(success_runs / total_runs * 100):.1f}%" if total_runs > 0 else "100%",

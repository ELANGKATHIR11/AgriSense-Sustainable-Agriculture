...
from backend.market_intelligence.analytics import (
    compute_analytics,
)

# Use the shared PostgreSQL sync engine — tables already exist
...
def db_session():
    """
    Provide a per-test PostgreSQL session.
    Rolls back after each test to keep the database clean.
    """
    db = TestingSessionLocal()
    try:
        yield db
    finally:
        db.rollback()  # rollback instead of dropping tables
        db.close()
...
def test_cache_mechanism(db_session):
    """Verify multi-level caching (memory + PostgreSQL)."""
...
def test_analytics_computations(db_session):
    ...
    p3 = MarketPrice(
        crop="Tomato", market="Mandi A", state="State A", price=5000.0, timestamp=now
    )

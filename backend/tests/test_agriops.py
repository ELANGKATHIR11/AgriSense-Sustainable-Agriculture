...
from backend.database.models import ModelRegistry, SensorReading, AIAgent, Task, Document
...
def test_mlops_champion_challenger(db):
    ...
    db.query(ModelRegistry).filter(ModelRegistry.id.in_(["mr-test-01", "mr-test-02"])).delete()
    db.commit()
    ...
    eval_res = asyncio.run(mlops_service.evaluate_champion_challenger(db, "crop_recommendation"))
...
def test_llmops_document_indexing(db):
    ...
    res = asyncio.run(llmops_service.index_document_chunk(
        db,
...
def test_agentops_task_running(db):
    ...
    res = asyncio.run(agentops_service.run_agent_task(
        db,

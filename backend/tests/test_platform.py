...
from backend.models import SensorReading, ModelRegistry
from ml.extended_isolation_forest import ExtendedIsolationForest
from ml.inference import predict_crop, predict_fertilizer, predict_yield, predict_irrigation, predict_disease_risk
...
def test_ml_inference():
    ...
    crop_res = predict_crop(N=50, P=40, K=40, temperature=28.0, humidity=60.0, ph=6.5, rainfall=100.0)
    assert "crops" in crop_res
    assert len(crop_res["crops"]) > 0
    assert "name" in crop_res["crops"][0]

    # 2. Fertilizer recommendation
    fert_res = predict_fertilizer(temperature=28.0, humidity=60.0, moisture=38.0, soil_type="Loamy", crop_type="Rice", nitrogen=50, potassium=40, phosphorus=40)
    assert "recommendedFertilizer" in fert_res
    assert "confidence" in fert_res

    # 3. Yield prediction
    yield_res = predict_yield(area_acres=10.0, avg_rainfall=1000.0, avg_temp=28.0, crop_type="Rice", nitrogen=50, phosphorus=40, potassium=40)
    assert "predictedYieldTons" in yield_res
    assert yield_res["predictedYieldTons"] > 0

    # 4. Irrigation prediction
    irr_res = predict_irrigation(moisture=38.0, temperature=28.0, humidity=60.0, crop_type="Rice")
    assert "waterRequiredLiters" in irr_res

    # 5. Disease risk prediction
    disease_res = predict_disease_risk(humidity=60.0, temperature=28.0, leaf_wetness_hours=8.0, soil_moisture=38.0, rainfall_mm=5.0)
...
def test_rag_engine():
    ...
    import asyncio
    results = asyncio.run(rag_engine.query_disease_kb("Tomato Leaf Mold symptoms", top_k=1))

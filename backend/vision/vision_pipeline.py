...
async def process_and_analyze_image(
    ...
    if not check_image_resolution(image, 128, 128):
        warnings.append("Resolution is lower than 128x128 threshold. Analysis precision may degrade.")

    # 3. Blur detection check
    is_blurry, blur_var = estimate_blur(image)
    if is_blurry:
        warnings.append(f"Image might be blurry (variance: {blur_var:.2f}). Consider recapturing in sharp lighting.")

...
def adapt_nutrient_to_twin(deficiency: str, severity: str):
    ...
    depletion = 5 if severity.lower() == "low" else (12 if severity.lower() == "medium" else 20)
    
    def_lower = deficiency.lower()
    if "nitrogen" in def_lower:
        state["soilTwin"]["nitrogen"] = max(10, state["soilTwin"]["nitrogen"] - depletion)
    elif "phosphorus" in def_lower:
        state["soilTwin"]["phosphorus"] = max(10, state["soilTwin"]["phosphorus"] - depletion)
    elif "potassium" in def_lower:
        state["soilTwin"]["potassium"] = max(10, state["soilTwin"]["potassium"] - depletion)
...
def adapt_health_to_twin(health_score: float, stress_level: str):
    ...
    state = twin_engine.get_state()
    state["cropTwin"]["cropHealthScore"] = int(health_score * 100) if health_score <= 1.0 else int(health_score)
    
    # Adjust predicted yield multiplier based on plant health
    if stress_level.lower() == "high":
        state["cropTwin"]["predictedYieldMultiplier"] = max(0.5, state["cropTwin"]["predictedYieldMultiplier"] - 0.15)
    elif stress_level.lower() == "low":
        state["cropTwin"]["predictedYieldMultiplier"] = min(1.5, state["cropTwin"]["predictedYieldMultiplier"] + 0.05)

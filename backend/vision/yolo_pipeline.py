...
CLASSES = ["Leaf", "Fruit", "Stem", "Disease Lesions", "Pest", "Weed", "Damaged Regions"]
...
def generate_mock_detections(width: int, height: int) -> List[Dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "confidence": 88.0,
            "box": [int(width * 0.35), int(height * 0.4), int(width * 0.65), int(height * 0.68)],
            "severity": "Severe"
        },
        {
            "class_name": "Fruit",
            "confidence": 95.2,
            "box": [int(width * 0.7), int(height * 0.2), int(width * 0.88), int(height * 0.45)],
            "severity": "Healthy"
        }
    ]
...
async def detect_image(payload: ImageInput):
        ...
        try:
            ...
            for r in results:
                for box in r.boxes:
                    ...
                    label = model.names[cls_id] if cls_id < len(model.names) else "Region"
    ...
    return {"success": True, "detections": detections, "dimensions": {"width": w, "height": h}}
...
async def annotate_image(payload: ImageInput):
    ...
    return {"success": True, "annotatedImage": f"data:image/jpeg;base64,{annotated_b64}"}
...
@router.post("/report")
async def generate_multilingual_report(payload: ImageInput, accept_language: str = Header("en")):
    ...
    raw_text = f"AGRISENSE MULTILINGUAL DIAGNOSTIC REPORT\n"

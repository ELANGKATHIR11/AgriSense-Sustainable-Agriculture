...
class MRAGOrchestrator:
    ...
    def _init_collections(self):
        ...
        """
        collections_to_init = [
            "documents", "images", "diseases", "crops", "weeds", 
            "weather", "satellite", "sensors", "agent_memory", 
            "conversations", "research", "government_docs", "market_prices"
        ]
        ...
        image_schema = pa.schema([
            pa.field("vector", pa.list_(pa.float32(), 512)), # standard image embed dimension (e.g. ResNet/CLIP)
            pa.field("id", pa.string()),
                        ...
                        if "already exists" in str(e).lower():
                            try:
                                ...
                                if os.path.exists(path):
                                    shutil.rmtree(path)
                                self.db.create_table(col, schema=schema)
                                logger.info(f"Self-healed and created LanceDB collection: {col}")
            except Exception as e:
    ...
    def _migrate_legacy_data(self):
        ...
        try:
            ...
            if len(tbl) == 0:
                logger.info("Migrating legacy FAISS/NumPy knowledge base to LanceDB 'documents'...")
                data = []
                for idx, doc in enumerate(_KNOWLEDGE_BASE):
                    ...
                    data.append({
                        ...
                        "id": f"leg-{idx}",
                        "text": doc["text"],
                        "metadata": json.dumps({k: v for k, v in doc.items() if k not in ("text")}),
                        "timestamp": datetime.utcnow().isoformat() + "Z"
                    })
                tbl.add(data)
                logger.info(f"Successfully migrated {len(data)} legacy entries to LanceDB.")
        except Exception as e:
            logger.error(f"Legacy data migration failed: {e}")

    def search_collection(self, collection_name: str, query: str, k: int = 3, metadata_filter: Optional[str] = None) -> List[Dict[str, Any]]:
        """
    ...
    def index_document(self, collection_name: str, doc_id: str, text: str, metadata: Dict[str, Any]):
        """
    ...
    def get_orchestrated_mrag_context(self, query: str, sensor_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """

...
from backend.agents.architecture import (
    ArchitectAgent,
    SolutionArchitectAgent,
    APIArchitectAgent,
    DatabaseArchitectAgent,
)
from backend.agents.developer import (
    FullStackAgent,
    FrontendAgent,
    BackendAgent,
    APIAgent,
...
from backend.agents.data import (
    DataEngineerAgent,
    DataScientistAgent,
    DataValidationAgent,
    DataVersioningAgent,
)
from backend.agents.qa import (
    QAAgent,
    UnitTestAgent,
...
from backend.agents.operations import (
    DevOpsAgent,
    SREAgent,
    MonitoringAgent,
...
from backend.agents.autonomy import (
    BugFixAgent,
    RefactorAgent,
    DependencyAgent,
    SelfHealingAgent,
...
async def get_history(limit: int = 15):
    """
    Get the historic swarm workflow runs from PostgreSQL.
    """
...
async def search_memory(
    query: str = Query(..., description="Query string for semantic/keyword retrieval"),
):
    """
    Search agent execution memory logs in PostgreSQL.
    """
    ...
    for h in history:
        if (
            q in h["task"].lower()
            or q in h["agent"].lower()
            or q in json.dumps(h["result"]).lower()
        ):
            filtered.append(h)
...
async def get_metrics():
    ...
    mock_cost = round(
        mock_tokens * 0.0000015, 4
    )  # Edge Ollama is free, but this mimics cloud telemetry

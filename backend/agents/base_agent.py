...
class BaseAgent:
    def __init__(self, name: str, role: str, skills: List[str], tool_registry: Optional[Any] = None):
    ...
    def register_event_handler(self, event_name: str, handler: Callable[[Dict[str, Any]], None]):
    ...
    def get_system_prompt(self) -> str:
        tools_list = list(self.tool_registry.tools.keys()) if hasattr(self.tool_registry, "tools") else []
        return (
    ...
    async def execute_task(self, task: str) -> Dict[str, Any]:
        ...
        event_data = {"agent_name": self.name, "task": task, "timestamp": self.metrics["last_active"]}
        ...
        for step in range(5):
            ...
            while attempt < max_retries and not success:
                try:
                    result = await ask_qwen_coder(prompt, system=self.get_system_prompt())
                    if result:  # Check if response parsed correctly
                        success = True
                    else:
                        raise ValueError("Empty or malformed JSON returned from local assistant.")
                except Exception as e:
                    attempt += 1
                    self.metrics["retry_count"] += 1
                    logger.warning(f"[{self.name}] LLM invocation attempt {attempt} failed: {e}. Retrying...")
                    await asyncio.sleep(2 ** attempt)
            
            if not success:
                logger.error(f"[{self.name}] LLM invocation permanently failed after {max_retries} attempts.")
            ...
            try:
                if hasattr(self.tool_registry, "execute_tool"):
                    ...
                    tool_output = f"Tool execution registry unavailable."
                loop_context += f"\nAction: {action}\nInput: {action_input}\nResult: {tool_output}\n"
            except Exception as e:
        ...
        self.history.append({"task": task, "result": final_result, "time": exec_duration})
        return final_result
    ...
    async def run_diagnostics(self) -> bool:
        ...
        try:
            ...
            has_tools = hasattr(self.tool_registry, "tools") and len(self.tool_registry.tools) > 0
            ...
            logger.info(f"[{self.name}] Self-diagnostics run completed. Status: {'OK' if ok else 'FAIL'}")
            return ok
        except Exception as e:

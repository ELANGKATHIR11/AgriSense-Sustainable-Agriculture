...
from backend.models import MarketProduct, Vendor
...
async def get_recommendations(disease: str = "Tomato Leaf Mold", db: Session = Depends(get_db)):
    init_marketplace(db)
    if "mold" in disease.lower() or "blight" in disease.lower():
        # Recommend copper fungicide
        return db.query(MarketProduct).filter(MarketProduct.category == "pesticide").all()

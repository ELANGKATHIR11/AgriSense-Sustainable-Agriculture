...
class StorageOptimizer:
    ...
    def calculate_project_size(self) -> Tuple[int, float]:
        ...
        for root, dirs, files in os.walk(self.project_root):
            ...
            dirs[:] = [d for d in dirs if d not in ('node_modules', '.git', '.venv', 'venv', 'catboost_info', '__pycache__', 'dist')]
            for file in files:
    ...
    def prune_old_checkpoints(self, checkpoint_dir: str = "ml/models/checkpoints") -> dict:
        ...
        chk_files = glob.glob(os.path.join(full_dir, "*.bin")) + \
    ...
    def run_optimization_cycle(self) -> dict:
        ...
        report_path = os.path.join(self.project_root, "ml", "models", "storage_report.json")

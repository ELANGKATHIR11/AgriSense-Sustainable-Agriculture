...
from backend.models import Farm, Field
...
def init_farms(db: Session):
    if db.query(Farm).count() == 0:
        f1 = Farm(
            name="North Grid Sector-A", location="Punjab Northern Belt", owner_id=1
        )
        f2 = Farm(
            name="Eastern Grain Hectare", location="Sikkim Valley Zone", owner_id=1
        )
        db.add_all([f1, f2])
        db.commit()
        db.refresh(f1)
        ...
        # Seed fields
        field1 = Field(
            name="Punjab Wheat Field", area_acres=45.0, crop_type="Wheat", farm_id=f1.id
        )
        field2 = Field(
            name=" पंजाब Maize Field", area_acres=20.0, crop_type="Maize", farm_id=f1.id
        )
        field3 = Field(
            name="Sikkim Organic Tea", area_acres=15.0, crop_type="Rice", farm_id=f2.id
        )

...
var sensorReadings = [
  { id: "1", deviceId: "ESP32-S01", timestamp: new Date(Date.now() - 36e5 * 5).toISOString(), soilMoisture: 42.5, temperature: 27.8, humidity: 62.1, pH: 6.4, nitrogen: 45, phosphorus: 38, potassium: 42 },
  { id: "2", deviceId: "ESP32-S01", timestamp: new Date(Date.now() - 36e5 * 4).toISOString(), soilMoisture: 41.2, temperature: 28.1, humidity: 61.5, pH: 6.4, nitrogen: 46, phosphorus: 37, potassium: 43 },
  { id: "3", deviceId: "ESP32-S01", timestamp: new Date(Date.now() - 36e5 * 3).toISOString(), soilMoisture: 39.8, temperature: 28.5, humidity: 60.2, pH: 6.3, nitrogen: 45, phosphorus: 39, potassium: 41 },
  { id: "4", deviceId: "ESP32-S01", timestamp: new Date(Date.now() - 36e5 * 2).toISOString(), soilMoisture: 38.3, temperature: 28.9, humidity: 59.4, pH: 6.3, nitrogen: 47, phosphorus: 38, potassium: 42 },
  { id: "5", deviceId: "ESP32-S01", timestamp: new Date(Date.now() - 36e5 * 1).toISOString(), soilMoisture: 37.1, temperature: 29.2, humidity: 58.8, pH: 6.3, nitrogen: 44, phosphorus: 38, potassium: 44 }
];
var modelRegistry = [
...
var predictionLogs = [
  { id: "pl-1", timestamp: new Date(Date.now() - 6e4 * 30).toISOString(), modelName: "CropRecommendation-XGBoost", inputs: { N: 50, P: 40, K: 40, pH: 6.5, temp: 28, hum: 60, rainfall: 120 }, output: "Maize (92% confidence)", latencyMs: 15, confidence: 0.92, driftScore: 0.04 },
  { id: "pl-2", timestamp: new Date(Date.now() - 6e4 * 20).toISOString(), modelName: "IrrigationIrrig-RandomForest", inputs: { moisture: 35, temp: 29, hum: 58 }, output: "Irrigation recommended: 1200L/acre", latencyMs: 20, confidence: 0.94, driftScore: 0.08 },
  { id: "pl-3", timestamp: new Date(Date.now() - 6e4 * 10).toISOString(), modelName: "PlantDisease-SmolVLM-3B", inputs: { imageUploaded: true }, output: "Tomato Late Blight (91% confidence)", latencyMs: 190, confidence: 0.91, driftScore: 0.01 }
];
function simulateCropRecommendation(input) {
  ...
  cropsList.forEach((crop) => {
    let matchScore = 100;
    if (crop.name === "Rice" && (pH < 5 || pH > 7)) matchScore -= 25;
    if (crop.name === "Maize" && (pH < 5.5 || pH > 7.5)) matchScore -= 20;
    if (crop.name === "Chickpea" && (pH < 6 || pH > 8)) matchScore -= 30;
  ...
  return {
    ...
    optimalPH: pH >= 6 && pH <= 7 ? "Healthy neutral optimal pH zone" : pH < 6 ? "Acidic sub-level; consider agricultural lime addition." : "Alkaline level; soil sulfur recommended.",
...
function simulateIrrigationOptimization(input) {
...
function simulateYieldPrediction(input) {
  ...
  if (cropType === "Soybeans") baseYieldPerAcre = 2;
  let tempDiff = Math.abs(avgTemp - 25);
  let tempFactor = Math.max(0.7, 1 - tempDiff * 0.02);
  let rainFactor = 1;
...
app.post("/api/mlops/retrain", (req, res) => {
  ...
  if (modelIdx !== -1) {
    modelRegistry[modelIdx].lastRetrained = (/* @__PURE__ */ new Date()).toISOString();
    modelRegistry[modelIdx].accuracy = parseFloat((modelRegistry[modelIdx].accuracy + (Math.random() * 0.012 - 4e-3)).toFixed(3));
    modelRegistry[modelIdx].f1Score = parseFloat((modelRegistry[modelIdx].f1Score + (Math.random() * 0.01 - 3e-3)).toFixed(3));
...
function calculateFAO56ET0(T, RH, windSpeed, Rn = 15) {
  ...
  return parseFloat(Math.max(1, Math.min(10, et0)).toFixed(2));
}
function predictMoistureWithAICorrection(currentMoisture, et0, rain, irrigation, daysAhead) {
  let predictions = [];
  ...
  for (let d = 1; d <= daysAhead; d++) {
    ...
    tempM = parseFloat(Math.max(12, Math.min(90, physicsMoisturePredict + aiCorrection)).toFixed(1));
    predictions.push(tempM);
  }
  return predictions;
}
function detectSensorAnomalies(humidity, moisture, temp, pH, nitrogen) {
  const alerts = [];
  if (moisture < 15) {
  ...
  if (temp < 4 || temp > 47) {
    alerts.push(`CRITICAL ANOMALY: Extreme thermal outlier logged (${temp}\xB0C). Potential thermal exposure.`);
  }
  if (pH < 4.5 || pH > 9.2) {
...
var currentTwinState = {
  ...
  waterTwin: {
    ...
    drainageRate: 0.6,
    waterBalanceHistory: [
      { day: "Mon", rainfall: 0, irrigation: 0, et0: 4.2, activeMoisture: 42.5 },
      { day: "Tue", rainfall: 0, irrigation: 0, et0: 4.5, activeMoisture: 41.2 },
      { day: "Wed", rainfall: 0, irrigation: 0, et0: 4.8, activeMoisture: 39.8 },
      { day: "Thu", rainfall: 1.2, irrigation: 1200, et0: 4.4, activeMoisture: 44.5 },
      { day: "Fri", rainfall: 0, irrigation: 0, et0: 4.7, activeMoisture: 41.2 },
      { day: "Sat", rainfall: 0, irrigation: 0, et0: 4.9, activeMoisture: 38.3 }
    ]
  },
  soilTwin: {
    nitrogen: 45,
  ...
  cropTwin: {
    ...
    growthTimeline: [
      ...
      { stage: "Yield Formation", expectedBiomass: 6e3, actualBiomass: 0 }
    ]
  },
  weatherTwin: {
    currentTemp: 28.5,
    ...
    forecast: [
      { date: "Wednesday", temperature: 28.5, humidity: 62, rainfall: 0, condition: "sunny", windSpeed: 8.4 },
      { date: "Thursday", temperature: 29.1, humidity: 58.5, rainfall: 1.2, condition: "cloudy", windSpeed: 9.6 },
      { date: "Friday", temperature: 24.8, humidity: 82, rainfall: 24.5, condition: "stormy", windSpeed: 18.2 },
      { date: "Saturday", temperature: 26.2, humidity: 75.4, rainfall: 4.8, condition: "rainy", windSpeed: 12 },
      { date: "Sunday", temperature: 27.9, humidity: 64.1, rainfall: 0, condition: "sunny", windSpeed: 7.8 },
      { date: "Monday", temperature: 28.2, humidity: 60.5, rainfall: 0, condition: "sunny", windSpeed: 8 },
      { date: "Tuesday", temperature: 28.8, humidity: 59, rainfall: 0, condition: "sunny", windSpeed: 8.5 }
    ],
...
app.post("/api/twin/update", (req, res) => {
  try {
    ...
    const currentRain = typeof rainfall === "number" ? rainfall : 0;
    ...
    currentTwinState.timestamp = (/* @__PURE__ */ new Date()).toISOString();
    ...
    currentTwinState.waterTwin.irrigationRecommendation = waterDeficitLiters > 0 ? `Moisture deficit flagged. Irrigate immediately with ${waterDeficitLiters} Liters on active grid.` : "Moisture saturation within optimal range. No irrigation required.";
  ...
  try {
    ...
    if (scenarioId === "drought_5_days") {
      ...
      for (let i = 1; i <= 5; i++) {
        timeline.push({
          day: i,
          soilMoisture: 40 + Math.sin(i) * 0.5,
